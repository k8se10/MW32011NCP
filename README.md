...
Several buttons are still unassigned (see the control map below), menu/UI navigation
...
| Start | Pause / menu toggle | ⬜ Not yet implemented |
...
- **Y / Start (one-shot commands):** `weapnext`, `togglemenu`, and `toggleprone` aren't
  held kbuttons at all — they're one-shot console commands. The engine's own
  command-execution function (a `Cbuf_AddText`/`Cmd_ExecuteString`-equivalent) hasn't
  been located yet; one candidate lead (`FUN_004d6da0`) was traced and ruled out — it
  turned out to be a HUD keybind-display formatter, not a dispatcher. See
  `re_notes/iw5sp.md` for the full trace.
...
- **Menu/UI navigation:** a genuinely separate system from in-game movement/look (menus
  read keyboard/mouse binds, not `usercmd`). Deliberately saved for last per the
...
our proxy d3d9.dll                         ← real injection point, ships beside the exe
    ...
    │  hooks IDirect3DDevice9::Present for a per-frame tick
    ▼
XInput poll (linked by us, game has none)  → deadzone + response curve
    ▼
pattern-scanned hooks into the engine's real per-frame pipeline:
    │  FUN_0057de60  — per-frame finalize step: movement, look, buttons all inject here
    │  FUN_00644ed0 / FUN_00643ce0 — Pmove entry points, real pm_flags bit for Sprint
    │  real KeyDown/KeyUp kbutton calls — ADS, Reload (not raw usercmd bits)
    ▼
separate hook/path still needed for menu & UI navigation (not implemented yet)
```
...
- Controller menu/UI navigation (D-pad/stick item selection, button-glyph prompts, a
  ...
  normally alongside controller gameplay (movement/look/buttons/ADS/Sprint/Reload no
  longer depend on any state the menu system also uses, so the two don't conflict).
