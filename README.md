...
> policy action, see the notice at the top of this file and `LTS_POLICY.md`).**
> This section is kept for historical context on what was distributed and why,
> not as a current recommendation — nothing below reflects an active support
> commitment any more.
>
...
policy, promotion process, and current status. **All LTS support for the `-x86`
line was withdrawn effective 2026-09-03 (emergency policy action — see the
notice at the top of this file)**; `v0.2.2-x86` was the LTS release and remains
the oldest release this project will ever designate LTS (the risk-mitigation release
...
- **Hold Breath** (L3 while ADS'd on a sniper) — real console/keyboard shares this
  ...
  trail.
- **Auto-mantle — confirmed working (v0.3.4), still off by default by design**
  ...
  unrelated concern — found and decoupled in v0.3.4. Direct confirmation after the
...
- **Auto-mantle while sprinting — confirmed working (v0.3.4), still shipped
  ...
  own icon lookup to succeed) found and decoupled in v0.3.4. See
...
This project also embeds the following font, bundled directly in the proxy DLL (`proxy_d3d9/proxy_d3d9.rc`) as a private, in-process-only font so it never depends on being installed on the user's system. Two variants are bundled — see the Config section below for how each is used and how to point either at a different, system-installed font instead:

- **[Isotherm Sans](https://github.com/k8se10/isotherm-sans)** (UI, Condensed, and Italic styles) — a modernized derivative of [Manrope](https://github.com/sharanda/manrope) (Copyright 2018 The Manrope Project Authors). SIL Open Font License, Version 1.1 (see `assets/fonts/IsothermSans-OFL.txt`). Switched to as the bundled default 2026-08-24, replacing the previously-bundled Barlow Condensed SemiBold (Copyright 2017 The Barlow Project Authors, also OFL 1.1) — Barlow is no longer bundled with this project. "UI" is the general default (most on-screen text); "Condensed" is used only by specific hint call sites (throwback prompt, sentry gun placement) that specifically want it. The bundled "UI" .ttf files (`assets/fonts/IsothermSans-UI.ttf`/`-UI-Italic.ttf`) are this project's own copies with their internal font-name table renamed (via fonttools) to "Isotherm Sans UI" so they're independently selectable from the Condensed pair under plain GDI font lookup — the glyph outlines themselves are untouched, unmodified upstream art.
