...
of every piece of text/glyph this mod renders). Build-verified working on
the current x64 line — the loading infrastructure itself is fully
architecture-neutral plain C.
...
mod's own code deliberately never uses on itself** — this project has a
hard-line, permanent policy against ever reading live gameplay-entity memory
in its own default code paths (see `CODE_STANDARDS.md`'s "Input Validation &
Security" section). That policy is unchanged and does not apply to plugins
...
`[plugin-loader]` lines to confirm what was found/loaded/rejected. A plugin
DLL must be built for the same bitness as the host — **x64**, matching the
...
vehicle for the plugin API itself. Build the project (`x64` configuration),
copy `rgb_text_plugin.dll` into your `plugins` folder, set
`[Plugins] Enabled=1`.
