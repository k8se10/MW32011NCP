...
8. **New crash/freeze report, root cause NOT YET confirmed — but a real, unrelated, serious bug was found and fixed along the way.** Live-reported as a severe regression ("crashes faster than ever in all modes," "acted very much like a memory leak, started fine then immediately freezes and closes"). Checked Windows Event Viewer directly: this is a genuine `WATCHDOG_VIOLATION` (bug check `0x141`) kernel-level event, not a normal application crash — materially different from every other crash this project has diagnosed to date. Ruled out both leading theories for this specific session: the deployed build couldn't have contained this session's in-progress `CustomResolutionWidth`/`Height` work (unbuilt at the time), and `InternalRenderScalePercent` was confirmed OFF the entire session via the log. **Found instead**: `Hook_DrawGlyphText`'s `[hud-font-id][gate]` diagnostic was never actually gated by its own documented `HudFontIdLogging` toggle — produced 76% (49,525 of 65,318 lines) of one session's log with that toggle OFF. Fixed. Not confirmed as the actual root cause, but a real, serious, unconditional cost removed regardless. `ResourceUsageLogging` (real memory/VRAM telemetry, already built, unused this session) is now on for the next test. See `known_issues.md` issue #105.
