...
7. **[Video] FpsLimitEnabled/FpsLimitTargetFps/FpsLimitEnhancementsOnly — built, live-tested, FAILED, reverted to off.** An in-mod frame-pacing limiter (issue #99 follow-up), intended as a same-purpose replacement for an external tool like RivaTuner Statistics Server. Live-tested immediately after shipping: "our limiter makes horrible pacing... its mad delay too input lagh... thats with rtss off." The wait was placed right after `Hook_EndScene`'s real `EndScene` call returns, assumed to be effectively a post-Present wait — likely wrong: `EndScene` and the real `Present` call are probably two separate D3D9 calls in this engine, with `Present` invoked by the game's own loop AFTER `EndScene` returns, meaning the wait actually landed BEFORE the real display event (a front-edge wait, not the intended back-edge one) — a direct, sufficient explanation for both the added latency and the bad pacing. Immediately reverted live (config hot-reload), confirmed clean by the user ("confirmed when toggled off immediately fine") — ships off by default in code regardless, so no install besides the live test machine was ever affected. Needs a redesigned hook point (this codebase's own Present hook is separately confirmed unreliable, likely Steam Overlay contention) before it's attempted again — not re-enabled until that's done and independently verified. See `known_issues.md` issue #99 for the full trail.
