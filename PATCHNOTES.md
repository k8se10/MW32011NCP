...
on an unarmored hit) was also root-caused and fixed (a real D3D9 stream-source
state leak in the shared full-screen draw helper) — motion blur stays labeled
**EXPERIMENTAL** pending independent live confirmation of that fix, see
`known_issues.md` issue #100 for the full trail. Separately root-caused this engine's own
...
7. **Motion blur broke native UI (including the damage-taken red-screen vignette specifically) when taking an unarmored hit — a real D3D9 state leak, not a state-corruption mystery.** `DrawFullScreenPass` calls `DrawPrimitiveUP` to draw its full-screen quad — officially documented Microsoft D3D9 behavior leaves the device's stream-0 vertex buffer binding undefined afterward, and this function never reset it. Motion blur's own hook fires *before* the native queued 2D/HUD-element dispatch runs each frame (that dispatch only has real work on frames where something's actually queued — e.g. a damage vignette), so whatever native UI drew right after inherited garbage stream-0 state. Live-diagnosed via a new dev-only render-thread diagnostic plus direct user isolation testing (ruled out `MotionBlurCenterFalloff`/blur shape as the cause; confirmed isolated to motion blur specifically, not FSR, which calls the same function but only after all native UI dispatching is already done that frame). Fixed by saving/restoring the real stream-0 binding around the draw call. Build clean, deployed — **not yet independently live-confirmed**. See `known_issues.md` issue #100 for the complete investigation trail.
