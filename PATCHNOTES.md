...
16. **`LEVELS_BUTTON_LIST` (Campaign Act mission list, both the top-level and per-mission sub-list depths) recalibrated via a real in-game F2/F3 editor capture.** Part of a pre-release Campaign/Spec Ops glyph-coverage pass — superseded the earlier screenshot/estimate-derived positions with real dragged values against the live screen. Both depths added to `kVerifiedGlyphGroups`. Build clean, deployed. More entries from this same pass to follow as the playthrough continues.
17. **Two new real Campaign pause-menu confirm-dialog glyph positions added** (`SWF_COMMON_DESC_RESIZE_POPUP_NAME` depth=3, `RESUME_POPUP` depth=3) — same live editor pass as item 16. Both are real, distinct screens reached specifically via the Campaign pause menu, correctly ordered ahead of each group's existing depth-independent fallback so they actually take effect. A third capture from the same session (a further `SWF_COMMON_DESC_RESIZE_POPUP_NAME` depth=1 variant) was deliberately NOT integrated — that group's depth=1 slot already discriminates between two real screens by description text, and adding an undiscriminated third entry risked silently breaking the working "Restart Mission" case; the real values are preserved in `known_issues.md` pending a text discriminator. `CAMPAIGN_BUTTON_LIST`/`PAUSE_LIST` were also independently reconfirmed this pass (fresh captures matched the existing entries exactly). Build clean, deployed.
