...
5. **[Video] FpsLimitEnabled/FpsLimitTargetFps/FpsLimitEnhancementsOnly — PREVIEW/WIP, off by default.** Direct follow-up to issue #99's vsync finding: an in-mod frame-pacing limiter, same purpose as an external tool like RivaTuner Statistics Server, so players don't need one just to get clean camera pacing on this engine. `FpsLimitTargetFps` (default 60) sets the cap; `FpsLimitEnhancementsOnly` (default on) restricts the cap to when a graphics-enhancement feature (motion blur, FSR sharpen, or internal render scale) is actually engaged, leaving normal gameplay uncapped — set to off for a general-purpose limiter that applies all the time instead. Implemented as a post-Present hybrid Sleep+spin wait (`Hook_EndScene`'s own tail, after the real EndScene call returns, not before — keeps the just-rendered frame's presentation latency low and only delays the next frame's start) timed off `QueryPerformanceCounter`, the same technique real limiters use rather than a bare `Sleep()` (Windows' ~15.6ms default timer granularity makes that far too imprecise on its own). Builds clean, not yet live-tested. See `known_issues.md` issue #99.
