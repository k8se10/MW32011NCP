...
isolation test — a first theory (a D3D9 stream-source state leak) failed live
testing, before the real cause (`SetFVF` itself, replaced with
`SetVertexDeclaration`) was found and fixed — motion blur stays labeled
**EXPERIMENTAL** pending one more confirming playtest against this final
build, see `known_issues.md` issue #100 for the full trail. Separately root-caused this engine's own
...
7. **Motion blur broke native UI — specifically the "you are hurt, get to cover" low-health warning (progressive red-screen vignette + text) — real root cause found and fixed via a 9-round live isolation test.** Isolated purely to motion blur (not `MotionBlurCenterFalloff`/blur shape, not FSR). Bisected `DrawFullScreenPass`'s own save/set/restore sequence one piece at a time, live-confirmed each round, ruling out in order: the engine hook's mere existence, the backbuffer capture, the real draw command itself, the pixel shader bind, the forced full-screen viewport, and all four render states — before finding it: calling `SetFVF` from that function AT ALL, even restored to the exact original value immediately afterward, breaks the native warning (most likely because the native renderer tracks its own "last bound FVF" state separately from the real device value, and the call desyncs that cache in a way a plain restore can't fix). **Fix**: `DrawFullScreenPass` now uses `SetVertexDeclaration` — a genuinely separate D3D9 state slot — instead of `SetFVF`, so the native code's own FVF stays completely untouched throughout. An earlier theory (the shared stream-0 vertex buffer binding `DrawPrimitiveUP` officially leaves undefined) was tried first and **live-tested failed** ("not the fix, the issue persists") before this investigation found the real cause. **A real crash followed immediately on first deploy** — Event Viewer confirmed an access violation in the real system `d3d9.dll`, root-caused to the new `g_screenVertexDeclaration` cache never being added to `ReleaseAllCachedTextures()` (the same crash class this project has hit twice before with cached device-bound shaders/textures) — a dangling pointer to a destroyed COM object after any device Reset. Fixed with the same unconditional-release pattern already used for every other cached resource there. Build clean (0 warnings/0 errors), deployed — **not yet independently re-confirmed live against this specific final build**. See `known_issues.md` issue #100 for the complete trail.
