...
### Groundwork
1. **First three real x64 gameplay hooks: Sprint, Movement, and Look — ALL
   CONFIRMED WORKING LIVE.** Built on the x64 rebuild's now-confirmed-working
   ...
   inversion); Look writes right-stick pitch/yaw directly into the engine's
   real angle accumulators, mirroring x86's own current
   `InjectControllerLookAngles` design (bypasses the mouse-cvar pipeline
   entirely — controller look gets its own independent rate, not mouse
   emulation under the hood), including a new `SigScan::ResolveRipRelative`
   capability to resolve those accumulator globals without a hardcoded
   offset. Direct user confirmation after real playtests: "movement and
...
5. **REMOVED: `[Video] CustomResolutionWidth/CustomResolutionHeight`.** Community request (GitHub issue #3): custom monitor layouts and split-screen setups where a single native-resolution percentage (`InternalRenderScalePercent`) can't express what's needed. Shipped reusing that same feature's hook point for an independent internal render W/H — then live-tested (`3000x1440` on a `2560x1440` native display) and confirmed via real log evidence to have genuinely applied, but to no visible effect: that hook only ever overrides the *internal* 3D scene's supersampling resolution, which gets composited back into the real, unchanged output window. It was never going to fix the actual problem (clipping/black bars from a mismatched real output resolution), which needs the real `BackBufferWidth`/`Height` to change instead — a materially harder, still-unsolved problem (the closest known mechanism, `r_mode`+`vid_restart`, is already confirmed to crash reliably). Removed rather than left shipped-but-non-functional; noted as genuine future work for post-0.3.5. See `known_issues.md` issue #102.
6. **REMOVED: `[Video] ForceD3D9On12`.** Already shipped labeled "DO NOT USE — CONFIRMED UNSTABLE" (a real, reproducible black screen never fully root-caused). Real-world evidence now shows the damage from enabling it even once **outlives turning the setting back off** — a corrupted driver-level state that caused crashes in later sessions where this config value read false the entire time, only clearing up on its own after several hours. An "off by default" toggle isn't a real safeguard when its damage isn't undone by disabling it, so it's removed from the mod entirely rather than left as an opt-in option — same standing this project already applies to any feature found unsafe (see the permanently-removed aim-assist feature). `InternalRenderScalePercent` (above) also picked up a new, un-missable on-screen warning at high values, given this project's own 32-bit process is confirmed to have a hard 4GB address-space ceiling regardless of GPU VRAM size — not blocked, just no longer silently invisible. See `known_issues.md` issues #92/#105.
7. **REMOVED (again): `[Video] FpsLimitEnabled/FpsLimitTargetFps/FpsLimitEnhancementsOnly`.** This feature's second design (writing the game's own `com_maxfps` dvar, after the first hand-rolled-wait design already failed and was removed once) got its first real live test and confirmed via an actual FPS counter that it caps MENU framerate but not real gameplay framerate — the dvar write itself was correct and stayed applied the whole session, the engine just doesn't respect it the way this project's own decompile assumed by analogy to other idTech3 titles. Two independent designs for the same goal have now both failed live testing; removed entirely rather than guess at a third. Standing guidance remains: use an external limiter (RivaTuner Statistics Server) instead. **Known residue**: if you had this enabled, `com_maxfps` may still be sitting at your configured target in your own game profile — reset it via the in-game Advanced Video "Framerate" option if gameplay still feels capped after updating. See `known_issues.md` issue #99.
8. **[Video] ForceHighQualityShadows/ForceHighQualityLighting — off by default, not yet live-tested.** Direct user request: native (not ReShade-style) shadow/lighting/reflection improvements. A single decompile of this engine's own renderer dvar-registration function revealed its full, real dvar catalog with names and descriptions. `ForceHighQualityShadows` writes `sm_fastSunShadow=0` — a real, confirmed "Fast sun shadow" quality/perf toggle (default ON in the engine). `ForceHighQualityLighting` writes `r_cacheModelLighting`/`r_cacheSModelLighting=0` — both real, confirmed "speed up lighting by caching previous results" toggles, trading a real uncharacterized performance cost for fresher-every-frame lighting. Same `SetDvarBool` mechanism this project's existing `ForceAnisotropicFiltering` already uses. **Shadow-map resolution specifically was investigated in real depth** (the render targets are real and confirmed — `R_RENDERTARGET_SHADOWMAP_LARGE`/`_SMALL`, in the same creation system `InternalRenderScalePercent` already uses — but the actual creation call site wasn't found even after a full Ghidra analysis pass) and is not covered by these two toggles; reflections were investigated and confirmed as environment-map/cubemap based, not screen-space, but tuning them (intensity/exponent) is a look choice rather than a clear resolution/fidelity increase, so nothing was wired up there yet. `ConfigVersion` 39→40. Build clean, deployed. See `known_issues.md` issue #107 for the complete trail.
...
9. **`DrawFullScreenPass` (the shared full-screen shader-pass helper every visual-suite effect is built on) never fully saved/restored D3D9 device state — found while investigating whether device-state hygiene, not the hook location, was issue #96's real cause.** It never saved or restored the viewport at all, force-nulled texture stage 0 instead of restoring the actual prior binding (discarding the COM reference incorrectly), and never restored FVF. Fixed: the pass's own full-screen quad now explicitly forces a full-screen viewport for its draw (pre-transformed XYZRHW vertices are still constrained by the ambient viewport) then restores the genuine previous viewport; texture stage 0 and FVF are now properly saved and restored (with correct `Release()` on the saved texture reference) instead of discarded/left dirty. A real, independent bug kept regardless of its role in #96 — every current and future full-screen pass (FSR sharpen, motion blur) benefits.
10. **`frame_benchmark.cpp`'s rumble-time accumulator had a genuine, unguarded data race, and covered only 1 of this project's 6 real background threads.** `FrameBenchmark_AddRumbleMs` wrote `g_rumbleMsThisFrame` with zero locking — a real bug introduced when issue #87 moved vibration writes onto their own dedicated thread (this file predates that refactor and was never updated for it). Fixed with an `INIT_ONCE`-guarded `CRITICAL_SECTION` (not a plain bool guard — unlike this project's other per-subsystem locks, this lock's very first use can legitimately race between multiple threads with no single "creator" thread). Extended, per direct instruction ("we should add all threaded stuff since we ever introduced it for controller polling way back"), to cover all six of this project's real background threads (poll, vibration, hot-reload, log-flush, asset-write, resource-log) via a `grep CreateThread` audit — the CSV benchmark output now has real per-thread timing columns for all of them, not just the one the original tool happened to track.
11. **CRITICAL: FSR's full-screen sharpen pass had zero state gating of any kind — root cause of a real crash/hard-hang plus a loading-screen/cutscene black screen.** Live-reported as a severe escalation of an earlier, seemingly-cosmetic black-screen report: "the black screen in menus/loading and cutscenes is a critical issue it even made the game crash and process legit hang." Caught live via a `memdiff.exe livedump` capture session (its own `IsInLevel()` gate — the same distrusted heuristic under investigation for motion blur's menu/loading gates — was removed this session so it could capture a main-menu state at all): a 42-snapshot sequence showed a real, abrupt mass deallocation of ~855 memory regions right before the process exited, a fundamentally different signature (855 freed, only 19 newly mapped) from a normal level-transition's healthy churn (499 freed, 376 newly mapped at a nearby, non-fatal transition). Inspecting the freed regions directly (new `dumpregion.exe` tool) found structured mesh-vertex data and a pair of texture-cache-shaped blocks sharing an identical magic number — real asset-streaming/texture-cache content, not incidental heap noise. Direct user isolation testing narrowed it precisely: motion blur fully OFF still crashed; FSR *also* off (render-scale alone, still at 150%) was clean — implicating FSR specifically. The actual bug: unlike motion blur (gated on menu-active + in-level since issue #96/#97), FSR's own full-screen capture-and-redraw had never had any state gating at all, running unguarded through every screen state including background asset-streaming during gameplay. Fixed by applying the exact same gates motion blur already had. **Confirmed live by the reporting tester for all three symptoms**: the crash/hard-hang, the cutscene black screen, and the loading-screen black screen. See `known_issues.md` issue #103 for the complete investigation trail, including two new memory-forensics tools (`diffsnap.exe`, a streaming region/byte-diff between two snapshots that works around the existing `memdiff.exe diff` mode's confirmed failure on multi-GB captures, and `readaddr.exe`) built along the way.
12. **Motion blur broke native UI — specifically the "you are hurt, get to cover" low-health warning (progressive red-screen vignette + text) — real root cause found and fixed via a 9-round live isolation test.** Isolated purely to motion blur (not `MotionBlurCenterFalloff`/blur shape, not FSR). Bisected `DrawFullScreenPass`'s own save/set/restore sequence one piece at a time, live-confirmed each round, ruling out in order: the engine hook's mere existence, the backbuffer capture, the real draw command itself, the pixel shader bind, the forced full-screen viewport, and all four render states — before finding it: calling `SetFVF` from that function AT ALL, even restored to the exact original value immediately afterward, breaks the native warning. An earlier theory (the shared stream-0 vertex buffer binding `DrawPrimitiveUP` officially leaves undefined) was tried first and **live-tested failed** ("not the fix, the issue persists") before this investigation found the real cause. **First fix attempt — `SetVertexDeclaration` instead of `SetFVF` — caused a real, repeatable crash and was reverted.** Event Viewer confirmed an access violation in the real system `d3d9.dll`, same exact fault offset on two separate runs; a first theory (the new declaration cache missing from `ReleaseAllCachedTextures()`, a dangling-pointer-after-Reset bug this project has hit twice before) was fixed and **the identical crash recurred anyway**, confirmed via the deployed DLL's own build timestamp that the fix really was running — ruling that theory out. Rather than keep guessing live against a real crash, `SetVertexDeclaration` was fully removed. **Actual shipped fix**: `DrawFullScreenPass` now calls neither `SetFVF` nor `SetVertexDeclaration` at all, reproducing the exact isolation-test stage already proven both to fix the native warning and to never crash — this mod's own motion-blur quad occasionally renders with the wrong vertex format for that one pass specifically (a cosmetic-only "pixelated" look, already reported, right at the moment of an unarmored hit or level entry), a known tradeoff, not a safety issue. A real fix for that remaining cosmetic gap is unstarted follow-up work. Build clean (0 warnings/0 errors), deployed. **Confirmed live** after a full 4-wave Survival playtest: "tested did 4 waves, performant, stable, reliably working, no issues other than that seen." See `known_issues.md` issue #100 for the complete trail.
13. **FSR's own #103-class bug on "exit level to main menu" escalated to a 100% reproducible crash — found and fixed.** What started as a reported black screen turned out to be much worse: "new reproducable issue, quit to menu crashes and closes mw3 every time guaranteed." Direct isolation testing ("tested fsr off this time and no such issue") confirmed FSR specifically. Very likely the SAME crash originally (and incorrectly) attributed to the `SetVertexDeclaration` work above — identical Event Viewer signature, and it recurred even after `SetVertexDeclaration` was fully removed, which in hindsight is best explained by this project's own test workflow routinely quitting to menu between rebuilds. Real mechanism: `kInLevelFlagAddr` is a raw per-frame time delta, not a true level-state flag, and can plausibly still read as "in level" for a frame or two while a level is actively tearing down, letting FSR's capture-and-redraw run against a mid-teardown backbuffer — the same class of bug issue #103 already fixed, on a transition that fix doesn't cover. **Fix**: an additional, independent gate check added to both FSR and (defensively) motion blur — `clcState`, a real native screen-state enum rather than a heuristic, checked against the one value this project has already independently confirmed (`0` = menu/disconnected). Build clean (0 warnings/0 errors), deployed, **confirmed live against the exact repro: "fixed."** See `known_issues.md` issue #104.
14. **[Gyro] never generated into the default config .ini — a real bug, now fixed. Also: sensitivity lowered and a new "gyro only while ADS" toggle added, per this feature's first real live-test report.** `WriteDefaultConfig()`'s template simply never had a `[Gyro]` section — the keys worked if hand-typed in (confirmed via `ReadBool`/`ReadFloat`), but were undiscoverable to a player scanning the generated file, exactly what the first live tester with real DualSense hardware hit ("I had to manually add [gyro] Enabled = 1 it doesn't generate inside the config ini"). Same report also confirmed glyphs/Sony-style icons and gyro movement itself all work correctly on real hardware, flagged the default `Sensitivity` as "far too high" (lowered 1.0→0.25, a directional correction pending further live tuning), and requested gyro only activate while aiming — added as a new `[Gyro] OnlyWhileAds` toggle (default off, preserves the original always-on behavior). `ConfigVersion` 35→36. Build clean, deployed. **Not yet independently re-confirmed live.** See `known_issues.md` issue #76.
15. **Severe crash/freeze night, resolved on its own — real Windows kernel-level `WATCHDOG_VIOLATION` events, not a normal app crash, plus a real unrelated bug found and fixed along the way.** Live-reported as a severe regression ("crashes faster than ever in all modes," "acted very much like a memory leak, started fine then immediately freezes and closes"). Checked Windows Event Viewer directly: 96 `WATCHDOG_VIOLATION` (bug check `0x141`) kernel dumps across two bursts — materially different from every ordinary D3D9 access-violation crash this project has diagnosed before. Ruled out both leading theories for the specific session analyzed: this session's in-progress `CustomResolutionWidth`/`Height` work was unbuilt at the time, and `InternalRenderScalePercent` read OFF the entire session via the log. **Found instead**: `Hook_DrawGlyphText`'s `[hud-font-id][gate]` diagnostic was never actually gated by its own documented `HudFontIdLogging` toggle — produced 76% (49,525 of 65,318 lines) of one session's log with that toggle OFF. Fixed. **Real-world resolution**: the same mission that reliably crashed within 10 seconds now runs clean, with no code change in between — direct support for the leading theory, "it was culprit for crashes after setting was disabled, seems after some hours the memory cleared out returning stability," pointing at `ForceD3D9On12` leaving the system in a bad driver-level state that outlived the config being turned back off (the same class of corruption already documented once before, issue #91's `DXCache` finding). **`ForceD3D9On12` removed from the mod entirely as a direct result** — see the What's New entry above. `InternalRenderScalePercent`/`CustomResolutionWidth` also picked up a new, un-missable warning at high values, given this project's own 32-bit process has a confirmed hard 4GB address-space ceiling regardless of GPU VRAM size. See `known_issues.md` issues #92/#105.
16. **`LEVELS_BUTTON_LIST` (Campaign Act mission list, both the top-level and per-mission sub-list depths) recalibrated via a real in-game F2/F3 editor capture, confirmed correct live.** Part of a pre-release Campaign/Spec Ops glyph-coverage pass — superseded the earlier screenshot/estimate-derived positions with real dragged values against the live screen. Both depths added to `kVerifiedGlyphGroups`. A second part of the same capture batch (two Campaign pause-menu confirm popups) turned out wrong and was reverted separately — see `known_issues.md` issue #109; this entry only covers the part that's confirmed correct and shipped. Build clean, deployed.

### Documentation
1. **Real-world performance expectation: running every visual-enhancement feature at once is genuinely GPU-intensive.** Direct user account, with `InternalRenderScalePercent`/`CustomResolutionWidth`, `FsrSharpenEnabled`, `MotionBlurEnabled`, and `ForceAnisotropicFiltering` all active together: "it is VERY intensive to run all enhancements(still more than playable, just dont be surpised when your PC turns into a JET ENGINE." Not a bug — each of these already has a separately-confirmed real GPU cost of its own, and stacking them compounds that cost, exactly as expected. Still genuinely playable on reference hardware (RTX 2080 Ti); players on lower-end GPUs should expect to dial back individual settings rather than run everything maxed at once. See `known_issues.md` issue #106.
...
4. **New `[Experimental] DamageDiagLoggingEnabled` diagnostic (issue #100)** --
...
9. **Buy-station menu occasionally opens into a stuck "thinks it's open but isn't drawn" state when entered while crouched — single occurrence, live-reported during this same v0.3.5 confirmation stream, not yet reproduced on demand.** Direct quote: "the buy station menu sometimes bugs out when you crouch and enter it ... basically the menu thinks it open but requires the pause menu to show up i only saw the bug once and didnt happen when normally interacting with it." Did not reproduce on a normal (standing) buy-station entry in the same session. Plausibly related to issue #1's own menu-open gate bit (`0x10` at `0x00B36210`) or crouch's stance-lock per-frame usercmd forcing (issues #27/#42) colliding with the buy station's own opening transition on an unlucky frame — neither mechanism confirmed against a real repro yet. Not actionable until reproduced deliberately with diagnostics on. See `known_issues.md` issue #110.
