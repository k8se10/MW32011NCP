...
config comments; an in-mod alternative to RTSS was attempted twice — a
hand-rolled timing wait failed live testing (added real input latency) and
was fully removed, then rebuilt on the game's own native `com_maxfps` dvar
instead, not yet independently confirmed live. Also carries a new,
...
is now **removed from the mod entirely**, not left as an opt-in option; see
issues #92/#105. See the itemized entries below and
