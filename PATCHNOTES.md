...
(UI dropping out, including the damage-taken red-screen vignette specifically,
on an unarmored hit) remains open — a real D3D9 stream-source state leak in
the shared full-screen draw helper looked like a strong, well-evidenced fix
but failed live testing the same session ("not the fix, the issue persists")
— motion blur stays labeled **EXPERIMENTAL**, real cause still unidentified,
see `known_issues.md` issue #100 for the full trail. Separately root-caused this engine's own
...
8. **Motion blur breaks native UI, specifically the "you are hurt, get to cover" low-health warning (progressive red-screen vignette + text) — real bug, one fix attempted and DISPROVEN live, real cause still unidentified.** User pinpointed the exact broken element: with motion blur on, neither the reddening nor the text ever shows; with it off, both work correctly — isolated purely to motion blur (not `MotionBlurCenterFalloff`/blur shape, ruled out directly; not FSR). A first theory — `DrawPrimitiveUP` leaves the device's stream-0 vertex buffer binding officially undefined, and the shared `DrawFullScreenPass` helper never restored it, plausibly corrupting whatever native UI the queued 2D/HUD dispatch draws right after motion blur's early hook point — was built, deployed, and **failed live testing the same session** ("not the fix, the issue persists"). A staged isolation test was built instead — stage 1 (keep the engine hook firing exactly as normal, skip the actual draw entirely) **live-confirmed the same session** ("yep shows as it should"): rules out the hook's mere existence, it's specifically something the draw call itself does. Stage 2 (do the backbuffer capture, skip the render-state-change/redraw sequence after it) **also live-confirmed** ("yeah still shows as should") — the plain `StretchRect` capture is harmless too. Stage 3 (run every state save/set exactly as normal, skip only the actual draw command) **also live-confirmed** ("broken here") — it's specifically one of the state changes themselves, even though every one gets fully restored afterward, not the draw command. Stage 4 (skip binding the custom pixel shader only, keep everything else including the actual draw) **also live-confirmed** ("yeah broken still") — rules out the pixel shader bind, narrowing to sampler filters/render states/viewport/texture-FVF-bind. Stage 5 (skip forcing the full-screen viewport only — this exact function had a real, previously-fixed viewport bug once before) **also live-confirmed** ("its broken though blur is visible now") — rules out the viewport too, narrowing to sampler filters/render states/texture-FVF-bind. Stage 6 (skip the four render-state changes only — `ALPHABLENDENABLE` the strongest suspect, forced `FALSE` for our own "opaque overwrite" draw) **also live-confirmed** ("confirmed still borken") — rules out all four render states too, narrowing to just sampler filters or the texture0/FVF bind. Stage 7 (skip binding this mod's own capture texture + FVF only — the strongest remaining candidate, since it binds a real GPU resource rather than just a flag value) built the same pass, not yet tested — `[Experimental] MotionBlurDrawTestStage` (0/1/2/3/4/5/6/7). See `known_issues.md` issue #100 for the complete trail.
9. **FSR's own #103-class black-screen bug recurs specifically on "exit level to main menu" — reported, deliberately deferred, not yet investigated.** Direct user report mid-session: "also the fsr bug happens if you ever quit a game," clarified as the same class of visual failure issue #103 already fixed, but on a transition that fix apparently doesn't cover. User's own hypothesis: a timing/lag gap between the real transition and whichever gate (`IsMenuActive_Exported`/`kInLevelFlagAddr`) FSR checks. Not touched this pass — surfaced mid-isolation-test on a separate issue (#100), deliberately deferred to avoid conflating two live investigations in one build. See `known_issues.md` issue #104.
