...
8. **Motion blur breaks native UI, specifically the "you are hurt, get to cover" low-health warning (progressive red-screen vignette + text) — REAL ROOT CAUSE FOUND, real fix not yet built.** User pinpointed the exact broken element: with motion blur on, neither the reddening nor the text ever shows; with it off, both work correctly — isolated purely to motion blur, not `MotionBlurCenterFalloff`/blur shape, not FSR. A first theory (the shared `DrawFullScreenPass` helper never restoring the D3D9 stream-0 vertex buffer binding `DrawPrimitiveUP` officially leaves undefined) was built and **disproven live** ("not the fix, the issue persists"). A staged isolation test bisected the rest of `DrawFullScreenPass`'s own sequence instead — seven rounds, each live-confirmed the same session, individually ruling out the engine hook's mere existence, the backbuffer capture, the real draw command itself, the pixel shader bind, the forced full-screen viewport, and all four render states (`ZENABLE`/`LIGHTING`/`ALPHABLENDENABLE`/`CULLMODE`) — until stage 7 (skip binding this mod's own capture texture + FVF to stage 0 entirely) **fixed it**: "okay interesting, it fixed by now it looks wrong, like a pixelated low res mess... when you get damaged no armor" — the native warning draws correctly again; the "pixelated mess" (and a matching level-entry flash) is this mod's own motion-blur quad now drawing with the wrong texture/FVF, fully expected for this diagnostic stage, not a new bug. **Root cause**: binding this mod's own capture texture to stage 0 — even though fully restored to the original value afterward — breaks whatever native code draws the low-health warning, most likely because the native renderer tracks its own "last bound texture" state separately from the real device state (a common redundant-state-avoidance optimization) that a plain restore doesn't resync. Stage 8/9 (splitting texture0 vs FVF individually) built the same pass, not yet tested — `[Experimental] MotionBlurDrawTestStage` (0-9). Once the exact culprit is pinned down, the real fix is very likely: draw this mod's own quad through a texture stage the native renderer doesn't use (e.g. stage 1+ instead of 0), sidestepping the cache-desync entirely rather than fighting it. See `known_issues.md` issue #100 for the complete trail.
