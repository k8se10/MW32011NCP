...
A new `[Video] CustomResolutionWidth/CustomResolutionHeight` pair (community
request) reuses the render-scale mechanism for an independent W/H instead of
a single native-preserving percentage. See
issues #76/#92/#99/#102/#105/#106 and the itemized entries below and
...
5. **[Video] CustomResolutionWidth/CustomResolutionHeight — off by default (0), not yet live-tested.** Community request (GitHub issue #3): custom monitor layouts and split-screen setups where a single native-resolution percentage (`InternalRenderScalePercent`, above) can't express what's needed, since it always preserves your desktop's own aspect ratio. These two values, when both set, override it directly with an independent internal render width/height — reuses that same feature's existing, already-proven-safe mechanism (a one-shot pre-device-creation hook, zero `r_mode` writes, zero `vid_restart`, zero live device recreation) rather than anything new. Requires restarting the game to take effect, same as the percentage mode. See `known_issues.md` issue #102.
