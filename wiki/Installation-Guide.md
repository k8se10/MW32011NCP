...
> **⚠️ No release is currently distributed.** This project is being rebuilt from scratch against MW3's own recompiled 64-bit binaries — see [[Home]] for current status and [[Changelogs]] for progress.

## Requirements

- **Windows, x64** (matching the game's own current binaries) — this project does not run on macOS/Linux directly.
- **Retail Steam copy of Call of Duty: Modern Warfare 3 (2011)**, current version — this project has only been analyzed and tested against retail Steam's `iw5sp.exe` (Campaign/Survival). See [[Compatibility]] for what is and isn't supported.
...
To update, just replace `d3d9.dll` with the new version from the latest release. Your `mw3ncp_config.ini` carries forward automatically — any setting you already tuned is kept, and renamed/restructured keys are migrated to their new equivalent, so there's nothing to redo by hand.
