...
void WriteDefaultConfig(const char* path)
{
    ...
    fprintf(f,
        ...
        "; WARNING (issue #105, 2026-08-29): iw5sp.exe is a 32-bit process with a\n"
        "; hard 4GB address-space ceiling no code fix can raise (confirmed via its\n"
        "; own PE header). Real crashes/freezes have been reproduced at 250-300%%\n"
        "; on an 11GB GPU that was nowhere near its own VRAM limit -- a 32-bit D3D9\n"
        "; process practically can't use the full 4GB regardless of GPU size. Values\n"
        ...
        "; SMAA 1x (shape-aware edge smoothing; scene only, never the UI). Off by default.\n"
        "SmaaEnabled=%d\n"
        ...
        "; look. A real crash this feature originally caused, and a separate bug\n"
        "; (the native low-health warning dropping out while this was on) are both\n"
        "; root-caused, fixed, and confirmed live -- see known_issues.md issues\n"
        "; #96/#100. 0 = off, 1 = on.\n"
        "MotionBlurEnabled=%d\n"
        ...
        "; sharpness improvement a since-removed ForceD3D9On12 investigation (issue\n"
        ...
        "; to the game's own existing com_maxfps (0 = uncapped, unaffected). Live-tested\n"
        "; (2026-09-21) -- on by default. If you'd rather use a proven external\n"
        ...
        "; during a detected read burst. Live-tested (2026-09-21) -- on by default.\n"
        "; 0 = off, 1 = on (default).\n"
        ...
        "; TEMPORARY dev-only diagnostic that helped find and fix issue #100\n"
        "; (RESOLVED 2026-08-28) -- kept in case a similar damage-related bug ever\n"
        "; needs the same investigation again, not because it's still needed now.\n"
        "; 0 = off (default). When on, the instant a real hit is detected (a\n"
        "; plausible single-frame health decrease), logs one line per frame for ~2s\n"
        "; of native visionset/render-context state sampled from inside the render\n"
