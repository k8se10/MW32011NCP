...
#include "git_version.h" // generated at build time by gen_git_version.bat (proxy_d3d9.vcxproj pre-build step)
...
// Pre-1.0 dev-build watermark, derived from (build date - release version/commit) per direct instruction, not a
// hand-maintained string: __DATE__ is this specific compile's own date, kModVersionString is bumped per release
// (mod_config.h), and MW3NCP_GIT_COMMIT_HASH is the exact commit that built this DLL (git_version.h, regenerated by
// gen_git_version.bat before every build -- "unknown" if git isn't available, never a stale/wrong hash).
const char* GetBuildWatermarkString()
{
    static char s_watermark[128] = "";
    if (s_watermark[0]) return s_watermark;
    static const char* const kMonths[12] = { "Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec" };
    int month = 1;
    for (int i = 0; i < 12; ++i) if (strncmp(__DATE__, kMonths[i], 3) == 0) { month = i + 1; break; }
    int day = atoi(__DATE__ + 4);
    int year = atoi(__DATE__ + 7);
    sprintf_s(s_watermark, "MW32011NCP dev build - %02d/%02d/%04d - v%s (%s)",
              day, month, year, kModVersionString, MW3NCP_GIT_COMMIT_HASH);
    return s_watermark;
}
...
void LoadModConfig()
{
    ...
    ReadBool(path, "Experimental", "GpuCaptureEnabled", g_modConfig.gpuCaptureEnabled);
