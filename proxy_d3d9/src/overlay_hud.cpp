...
// cap MENU framerate but not actual gameplay framerate, despite the
// com_maxfps write itself being confirmed correct and staying applied the
// whole session -- the engine evidently treats com_maxfps differently once
// connected/simulating than this project's own decompile (issue #90) assumed
// by analogy, a mechanism that was never actually confirmed via decompile.
//
// KNOWN RESIDUE: any player who had this enabled may still have `com_maxfps`
// sitting at their configured target (e.g. 60) in their own `players2/
// config.cfg`, since this feature's own restore-on-deactivate logic (removed
// along with everything else here) never got a chance to run for a player
// who simply updates without one more relaunch first. Reset it via the game's
// own in-game Advanced Video "Framerate" option if gameplay still feels
// capped after this update -- not something this project's own removal can
// silently undo on its own.
...
// pacing) and REMOVED entirely. The wait was placed right after the real
// EndScene call returns, assumed to be effectively "post-Present" -- very
// likely wrong: EndScene and the real Present call are probably two separate
// D3D9 calls, with Present invoked by the game's own loop AFTER EndScene
// returns, meaning the wait actually landed BEFORE the real display event (a
// front-edge wait holding an already-rendered frame back, not the intended
// back-edge one). See known_issues.md issue #99 for the full trail. Use an
// external limiter (RivaTuner Statistics Server) instead -- do not
...
void DrawBuildWatermark(void* device)
{
    ...
    constexpr float kWmRightMarginPx = -40.0f; // moved 50px further right (was 10)
    constexpr float kWmBottomMarginPx = -15.0f; // moved 10px further down (was -5)
...
HRESULT WINAPI Hook_EndScene(void* device)
{
    ...
    // this file's own removed-code comment (where ApplyFpsLimitIfEnabled
    // used to live, right after ApplyForcedAnisotropicFilteringIfEnabled's
    // definition above).
...
void GetRealScreenSize(void* deviceIn, int& outWidth, int& outHeight)
{
    ...
    if ((g_modConfig.unboundedDevLog || s_resScaleLogLines < 20) && (outWidth != s_lastLoggedWidth || outHeight != s_lastLoggedHeight ||
