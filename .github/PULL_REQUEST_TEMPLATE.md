...
- [ ] Hook target resolved via runtime signature scanning (wildcarded byte pattern, resolved once at startup, cached) — not hardcoded, for the current `-x64` line (see `CODE_STANDARDS.md`); the frozen `-x86` line's own hooks remain historical hardcoded addresses
