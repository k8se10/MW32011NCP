...
| Predator Missile (`remote_missile`) | Black Tuesday, Down the Rabbit Hole (not yet playtested this session — Act 3) | ⚠️ Partial, per this project's own GSC trace (not this session's playtest) | Camera/view control confirmed working (shares the real UAV-control system), Fire launch uncertain — real hypothesis: Fire uses raw usercmd bits rather than the real `+attack` command dispatch the GSC script's `notifyonplayercommand` gate is listening for. See `known_issues.md` task #7 and `iw5sp.md`'s full GSC trace. **Also a real MP killstreak — see below.** |
