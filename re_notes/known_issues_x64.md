...
- **Real, un-actioned next steps for the differential question specifically**: (1) check whether `FUN_1401d7480`/`FUN_1401d7f70` are called MORE TIMES per frame while paused or on sunny maps (e.g. more secondary/PIP viewports processed) rather than each call costing more -- a call-count differential would explain the symptom without needing per-call cost to differ at all; (2) decompile `FUN_1401d8f70` (called immediately before the SSAO chain, not yet examined) in case it holds its own real per-frame setup cost; (3) the technique-selection dispatch (`ssao_calc_slow` vs `fast`, `apply_fullres` vs `downsampled`) remains genuinely unlocated -- confirmed routed through a hash/indirect lookup that defeated direct string-xref tracing, would need a `ScanStructRegion.java`-style struct-offset consumer scan or a different technique entirely to pin down.
