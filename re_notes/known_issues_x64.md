...
- **Revised near-term plan, materially different from (and more achievable than) the original item 2/3 framing**: `ReShade 6.8 Full Add-On Support` (direct, in-process — chained behind this project's own proxy `d3d9.dll` the same way FSR/motion blur/SMAA's own full-screen pass already works, not a separate injected process) + `LumeniteFX` (real motion vectors, no engine RE needed) + `DLSS5-Feeder` (the actual DLSS/DLAA feed) — no cross-process bridge, no fixed-function asset capture, no path tracing. This is DLAA/DLSS UPSCALING AND ANTI-ALIASING ONLY, not a "renderer replacement" in the geometry/lighting/material sense — a real, substantial visual/performance win, genuinely achievable, but a different and smaller scope than "renderer replacement" implies. **Real open questions, not yet investigated**: whether ReShade's own D3D9 hook coexists cleanly with this project's own existing D3D9 hooks (`EndScene`/`Reset`/`CreateDevice`, the full-screen capture/composite pipeline SMAA/FSR/motion blur already share) or needs explicit chaining/ordering work; ReShade's own real license/redistribution terms for bundling vs. requiring a player-side install; and whether `LumaFlow`'s optical-flow motion vectors are accurate enough for DLSS's own quality bar on this specific engine's motion characteristics (30Hz locked sim tick, per this project's own already-documented engine-performance findings) — untested, a real risk not a given.
