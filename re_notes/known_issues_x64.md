...
**Motion vectors, priority revised again same day, direct instruction: "we want proper motion vectors. we may have to surface them off of engine data."** Camera-only reprojection (`cameraMotionIncluded = eFalse`, reusing `sl_matrix_helpers.h`'s own real `calcCameraToPrevCamera`/`recalculateCameraMatrices` reference code against the same per-frame camera data this project's own motion-blur feature, issue #95, already produces) stays real and correct for static world geometry, and Streamline's own buffer semantics ("Object and **optional** camera motion vectors," confirmed directly from `ProgrammingGuide.md`) mean it composites with, rather than being replaced by, real per-object data — but a first release now targets real, engine-sourced per-object motion, not the camera-only baseline alone. The real, researched technique: capture the actual current+previous-frame vertex-shader constants (transform, and bone-matrix palette for skinned draws) IW5 already uploads via `SetVertexShaderConstantF`, then replay the SAME real vertex/index buffers through a small, hand-authored velocity shader pair this project compiles itself — not patching IW5's own precompiled shader bytecode, and not a generic optical-flow approximation. **The single most important unconfirmed lead**: whether IW5's own draw-command stream (`gen drawsurfs`, per `renderer_architecture_map.md` §5b — "generate draw surfaces," the classic id-Tech bridge from the culled/visible entity list to actual per-surface draw commands) carries a stable per-entity key this project could cache previous-frame transforms against, avoiding an unsafe draw-order-based cache key. A real, already-on-record, separately-identifiable `skin model` command-buffer stage (tag `0x10`, distinct from generic `cell dyn model`) is direct, already-confirmed evidence skinned draws are a first-class category in IW5's own command stream, not something needing fresh discovery. None of this is native-RE-blocked in principle — real, scoped, sequenced work (decompile `gen drawsurfs` for entity identity -> decompile `skin model` for the bone-palette convention -> author the velocity shader pair -> wire the capture/retain cache), not yet started. Full technical detail: `vulkan_dlss_pipeline_research.md` §2.6.
