...
**Retested same day -- round 1's fallback did NOT fix the pause failure, and introduced a real NEW regression on top: the previously-reliable LIVE-gameplay capture also failed this time ("the live one failed where it didnt before, thats a regression").** Two new tiny (~244KB) dumps, both logging `MiniDumpWriteDump FAILED even with fallback flags (first=2147942487, fallback=2147942487)` -- i.e. the round-1 FALLBACK combo (`MiniDumpWithDataSegs | MiniDumpWithPrivateReadWriteMemory`, with neither `MiniDumpWithHandleData` nor `MiniDumpWithFullMemoryInfo`) failed with the IDENTICAL `ERROR_INVALID_PARAMETER` as the full combo -- ruling those two flags out as the specific culprit, since a combo without either one still fails the same way. **Root cause of the live-capture regression, found on review: both attempts reused the SAME file handle.** A failed `MiniDumpWriteDump` call can partially write to the file before erroring, so retrying on that same handle risks feeding a corrupted/appended file into the second attempt -- plausibly explaining why even the live-gameplay case, previously reliable, broke once a same-handle retry was added. **Round 2 fix**: each of now THREE tiers (full combo, reduced combo, and a new bare `MiniDumpNormal` minimal tier -- no extra flags at all, the most universally-supported combination DbgHelp has, still enough for real thread stacks/module list/exception info) opens its own fresh, truncated (`CREATE_ALWAYS`) file handle rather than reusing one across attempts. Build-verified (x64 Release, 0 errors, `dumpbin`-confirmed genuine x64 output), deployed. **Retested same day -- succeeded, via the new `minimal` (`MiniDumpNormal`) tier, on BOTH captures. But the result reframes the whole failure: it is NOT pause-specific after all.** Both captures (`selfdump_20260925_155744.dmp`/`_155750.dmp`) were confirmed via surrounding log context to be during LIVE gameplay (`[x64-renderview-select-diag]` actively cycling view indices 6/7/8/1/10 each frame, `menuActiveGateFlag(DAT_1406e2550)=0x00000000`), not paused -- yet the `full` and `reduced` tiers both still failed with the identical `first=2147942487` (`ERROR_INVALID_PARAMETER`) before the `minimal` tier finally succeeded. The earlier framing ("fails specifically while paused") was wrong; the real correlate appears to be something about session STATE/TIME rather than pause vs. live -- the bounded combo had succeeded earlier in this exact same session (`selfdump_20260925_152317.dmp`, 2.7GB, ~15:23) before failing on every attempt since (~15:24 onward, pause AND live alike). One concrete, testable lead: `[resource-diag]` logged `workingSetMB=5535.4`/`privateBytesMB=5429.6` at the time of the successful capture -- real, substantial session memory growth compared to whatever the process footprint was at 15:23, plausibly from DXVK/Streamline shader-cache growth, texture-cache growth, or an actual leak, any of which could grow a private-memory region large or unusual enough to break `MiniDumpWithPrivateReadWriteMemory`'s enumeration once it crosses some threshold -- not yet confirmed, just the strongest lead so far. Both successful minimal-tier dumps were opened via `mcp-windbg` (static, offline, no live attach) and confirmed usable: real thread lists recovered, 59 live threads including a full DXVK/Vulkan worker pool (`dxvk-submit`/`dxvk-queue`/`dxvk-cs`/`dxvk-descriptor`, several `dxvk-shader-l`/`n`/`h` compiler threads, `dxvk-frame`) and an active Streamline `sl.log` thread -- real, independent confirmation of heavy background GPU-driver/Vulkan activity, consistent with (not proof of) the separately-reported GPU-loud-during-pause observation, though this specific pair of dumps was captured during live play, not pause. **Next step for a future round**: capture a fresh pair immediately after a clean relaunch (low memory footprint) to test whether the bounded/reduced tiers succeed again early in a session and only start failing once memory footprint grows past some point -- that would confirm or rule out the memory-growth theory directly.
