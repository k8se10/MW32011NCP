...
**Extensive further digging (5+ real attempts, forcing three more Ghidra function boundaries via the same padding-scan technique -- `FUN_006751c0`, and its own callees `FUN_004179d0`) did not find an x86 function that structurally matches `FUN_1401939f0`'s per-viewport, 6-call, category-gated (bloom/glow/lens-flare/fog/distance-fade) shape.** Every x86 pass-type table entry examined this round instead routes through one of two generic, heap/priority-queue-driven batch draw-submission functions (`FUN_0068f150`, reached via `FUN_00693a60`<-`FUN_004d3830`; `FUN_0068f0c0`/`FUN_0068eee0`, reached via `FUN_004179d0`<-`FUN_006751c0`) -- neither calls `FUN_0049bf50` (the activator) at all. **Working hypothesis, NOT yet proven**: x86 may achieve whatever visual effect `FUN_1401939f0` produces (if x86 has an equivalent effect at all) through these generic, single-pass batched draw-queue submissions rather than by re-invoking the expensive full render-view activator six separate times per viewport -- if true, this would mean the x64 recompile didn't just ADD calls at fixed orchestrator positions (points (a)/(b) above, both now fully confirmed) but restructured an efficient batched-submission pattern into six separate, much more expensive discrete re-activations for what may be the same underlying visual work. This is the single largest remaining open question in this whole investigation, and the most consequential if confirmed -- but per this project's own Fresh Perspective policy, five real, differently-angled attempts (viewport-triple caller tracing, jump-table entry decompiles, two separate large-function boundary recoveries) without a positive match is close to the threshold for stepping back rather than continuing to force more Ghidra boundaries blindly. Real next step, if resumed: rather than continuing to hunt for a name via more caller-tracing, target x86's own `FUN_0068f150`/`FUN_0068f0c0` directly -- decompile what their own function-pointer callbacks (`piVar7[2]`/`piVar7[3]` in `FUN_0068f150`) actually do, since that's where any real bloom/glow/lens-flare-equivalent drawing logic would have to live if this hypothesis is correct.
