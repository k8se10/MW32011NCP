...
Build-verified (x64 Release, 0 errors, `dumpbin`-confirmed genuine x64 output: `8664 machine (x64)`), deployed to the live install. `[Experimental] GpuSyncTimingLogging=1` set live in `mw3ncp_config.ini` per this project's own standing "set config toggles live, don't wait to be asked" convention. **Not yet live-tested** -- next step is a short live-gameplay capture and a short paused capture, both under `GraphicsApi=Vulkan`, comparing the logged `elapsedMs` values directly -- the real payoff step for this whole thread, not yet reached.
