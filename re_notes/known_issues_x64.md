...
**This is the strongest, most concrete, most directly actionable root-cause candidate found in this entire multi-day investigation** -- unlike the smaller, confirmed-but-modest per-frame-orchestrator additions (points (a)/(b) above) or the still-unquantified `FUN_1401939f0` question, this one directly explains a real, dominant, live-captured pattern (the 6/7 alternation accounting for over half the calls in the worst frames) with a specific, well-understood mechanism (a real hardware-capability check that appears to be entirely missing on the "always take the expensive path" side). **Not yet a confirmed fix** -- the honest remaining gaps: (1) confirm `DAT_021d35f4`'s real x64 struct-field equivalent doesn't exist under a DIFFERENT constant/technique (a broader search, e.g. for `CheckDeviceFormat`/`CheckDepthStencilMatch` vtable-call patterns generally, not just this one specific FourCC, would rule this out more thoroughly); (2) confirm what the "fast path" x86 takes INSTEAD actually is (i.e., what real, cheaper native shadow-rendering mechanism replaces the activator call when hardware is capable) -- not yet traced, needed before this becomes an implementable fix rather than a diagnosis; (3) independently verify on live hardware that this project's own reference GPU (RTX 2080 Ti, per this issue's own earlier render-scale testing) would in fact qualify as "capable" under x86's own detection (very likely, given it's a real, modern gaming GPU, but not yet directly tested). Real next step: find x86's real "fast path" shadow-rendering function (whatever runs when `DAT_021d35f4` is nonzero and this activator call is skipped) to understand what x64 would need to gain to recover it.
