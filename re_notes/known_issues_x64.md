...
**Implementation started, 2026-09-23 — real, build-verified first slice, DXVK itself not bundled yet.** `GraphicsApi` shipped exactly as designed above (`mod_config.h`/`.cpp`, config schema v44->v45): `LegacyD3D9` default, `Vulkan` opt-in. **SP-only, direct instruction the same day**: "make sure its conditional only SP for now as again we know mp holds more risk and this is qol not an essential feature for mp, so until we have real precedent from sp we will bring it to mp" — real, independently-enforced at two points: `d3d9_hook.cpp`'s `IsGraphicsApiVulkanModeAllowed()` (checked at `CreateDevice` time) and `dllmain.cpp`'s `TryLoadVendoredDxvk()` (checked before `d3d9.dll` itself loads — `DetectGameExecutable()` moved earlier in `DllMain` to make this possible). Real loading architecture implemented per the DXVK-maintainer-confirmed pattern (`vulkan_dlss_pipeline_research.md` §4.5 — a wrapper that calls DXVK's own `Direct3DCreate9` directly, not export-forwarding to a renamed DLL): when Vulkan mode is actually allowed, `TryLoadVendoredDxvk()` loads `dxvk\d3d9.dll` (next to this mod's own DLL) INSTEAD of the real system `d3d9.dll`, so every existing forwarded export routes through DXVK from that point on with zero other code changes needed. DXVK isn't vendored into the repo yet (a real, separate licensing/attribution/redistribution task — zlib license, real attribution required once shipped) — until it is, this always falls back to the real system `d3d9.dll`, loudly logged either way. Build-verified: x64 Release, 0 errors, `dumpbin` confirms genuine x64 output, deployed live. Real remaining work: vendor an actual DXVK build, then the real per-frame Streamline integration (jitter injection, motion vectors, `slSetVulkanInfo`, the Vulkan swapchain-interception question from §2.7) on top of this now-real loading foundation.
