...
**Roadmap re-checked against the full renderer-mapping/analysis session, 2026-09-24.** Direct instruction to continue the renderer-replacement roadmap with concrete next steps across the visual-enhancement suite (DLSS, FSR, RT). Full detail in `vulkan_dlss_pipeline_research.md`'s own "Open questions" section (items 2/3/8, updated same day) -- short summary here:
- **DLSS jitter injection is now the single highest-leverage next RE task.** `FUN_1401d8f70` (the one plausible lead for IW5's own projection-matrix-build function) is confirmed to sit inside the main-thread, pre-command-buffer per-player frame-setup chain -- real architectural context, not yet decompiled itself. This is the one remaining blocker between "contract fully specified" (already true) and "implementable."
- **Motion vectors: the per-entity identity question is RESOLVED** (`cell scene ent`/`add scene ent` confirmed to share one index space, gated by the engine's own "active this frame" flag -- exactly the sentinel Streamline's own motion-vector-invalid case needs), **but the earlier "`FUN_1401c7aa0` = dynamic models" candidate is weaker than previously stated** -- it and its two siblings all route through a shared spatial-cell-registration function, not a draw-dispatch path as originally framed. Stage 1 (camera-only reprojection) is unaffected and buildable today; Stage 2 (real per-object motion) needs this category question re-settled plus the skinned-draw bone-palette convention before it's actually buildable.
- **FSR: confirmed unaffected by any of this session's renderer findings** -- it's a pure post-process layer with zero dependency on the native command-buffer/render-target/threading architecture mapped this session. No new next step.
