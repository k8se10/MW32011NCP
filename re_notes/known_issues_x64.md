...
**Real, in-house alternative shipped same day instead of standing up new tooling**: rather than RenderDoc/PIX, `Hook_ProjectionMatrixBuild` now has a strictly opt-in `[Video] ProjectionMatrixJitterProbeEnabled` mode (default off, developer/RE-verification only, never a player-facing feature) that cycles a large (0.25), obviously-visible sine perturbation through every plausible candidate float slot (Row0[2]/[3], Row1[2]/[3], Row2[0]/[1], Row3[0]/[1]) one at a time, 4 seconds each, main-scene-pass only (the shadow pass is excluded via a real return-address discriminator -- `FUN_14018e720`'s two call sites to the matrix builder were disassembled and their exact return addresses computed: shadow=anchor+0x23E, main scene=anchor+0x2E1, resolved once at startup via a real signature-scanned anchor). Build-verified (x64 Release, 0 errors, genuine x64 confirmed via `dumpbin`), deployed -- **not yet live-tested**. This is a real, empirical, human-observed test (watch which candidate makes the screen visibly swim/warp) matching this project's own established "verify live" bar, not a guess shipped as a fix. Full trail: `vulkan_dlss_pipeline_research.md` item 2.
