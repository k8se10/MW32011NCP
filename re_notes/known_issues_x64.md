...
**Real data used directly instead, per user's own live-reported numbers from this exact capture: pause ~10fps (~100ms/frame), live gameplay ~17fps (~58.8ms/frame), both deliberately the regression-repro scenario, not casual play.** Pause ran ~1.7x slower than gameplay on the same content -- real, further, independent confirmation (a third data point, alongside the two Campaign live-tests and the sunny-Survival-map result already on record for `SkipRedundantShadowActivation`) that the pause-vs-live differential is real and still not fully closed by the shipped shadow-activation fix, even though this specific capture's `[gpu-sync-mark]` log can't be split into paused-vs-live halves after the fact (no reliable `clPaused` boundary to split on). The `elapsedMs` values already captured this session (steady ~34-36ms with 370ms/274ms hitch spikes, see above) are very likely a MIX of live and paused frames given the confirmed-both-states nature of this test, not a pure live-only baseline as originally assumed when they were first logged -- a real, honest correction to that earlier read of the same data.
