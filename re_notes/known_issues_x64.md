...
- **Real next step, not yet done**: trace `FUN_1401d9a10`'s 3 callers to find the actual per-frame invocation context (does this run once per viewport per frame, unconditionally, regardless of pause state? -- consistent with the standing "pause keeps re-rendering the full post-process pipeline" finding if so), and find the real consumer(s) of the marshaled struct's `+0x9fc`/`+0x9fe` SSAO-enable/downsample fields (the actual `ssao_calc_slow`-vs-`fast` and `apply_fullres`-vs-`downsampled` dispatch decision) -- likely needs a struct-offset consumer scan (`ScanStructRegion.java`-style) rather than more direct-reference tracing, since the technique dispatch itself is confirmed to route through the same kind of hash/indirect lookup that already defeated direct string-xref tracing above.
