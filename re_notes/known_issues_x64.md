...
- **Real, un-actioned next steps**: (1) the technique-selection dispatch itself (`ssao_calc_slow` vs `fast`, `apply_fullres` vs `downsampled`) remains genuinely unlocated -- confirmed routed through a hash/indirect lookup that defeated direct string-xref tracing; if per-call cost (not count) is the real differential, this is where it would have to live, and would need a `ScanStructRegion.java`-style struct-offset consumer scan or a different technique entirely to pin down; (2) decompile `FUN_1401d8f70` (called immediately before the SSAO chain in both `FUN_1401d7480`/`d7f70`, not yet examined) in case it holds its own real per-frame setup cost independent of SSAO; (3) step back from SSAO specifically and check whether ANY part of the per-frame pipeline this chain touches (the `FUN_1401ea4b0(1..6)`-bracketed stages) shows real per-stage cost variance between pause/live or sunny/normal -- the differential may live in a completely different stage than SSAO, which this thread has now reasonably ruled out as the primary suspect without more per-call-cost evidence.
