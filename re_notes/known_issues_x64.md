...
- [#2](#2-roadmap-anti-aliasing-and-a-modern-renderer-path-smaa--dlss--path-tracing--dx12vulkan) — ROADMAP: anti-aliasing and a modern-renderer path (SMAA -> DLSS/FSR2 -> path tracing -> DX12/Vulkan) — **Roadmap Idea; SMAA 1x built and shipped 2026-09-21, then PARKED off by default (capture/redraw path itself suspect, independent of the SMAA shaders); FXAA also shipped 2026-09-21 as a limited fallback; `GraphicsApi`/Vulkan/DLSS architecture LOCKED 2026-09-23, not yet implemented; publicly committed to on Nexus 2026-09-23 (Vulkan/RT/DLSS named directly) — see this entry's own "Public commitment" note**
