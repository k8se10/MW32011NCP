...
**Honest, not-yet-closed gap**: `$shadowmap_large`/`$shadowmap_small`'s own real creation code was NOT found in either of these two functions, and is not reachable via either a direct LEA or MOV-imm64 reference to their own name-string-pointer table entries anywhere in the `.text`/`.bind` sections (both searched, zero matches) -- meaning shadow-map creation is very likely reached via a genuinely separate, table-driven/indexed loop this session did not locate, the same class of Ghidra reference-manager blind spot already documented elsewhere in this project (a real data-driven dispatch the static reference manager can't see without the actual runtime index value). A full string search also found **zero** dedicated `r_shadowmap*`-style size dvar anywhere in the binary, consistent with this project's own multi-year-standing "shadow-map resolution specifically... not found" finding (`README.md`'s own known-gaps history, 9+ prior RE rounds) -- shadow-map sizing is very likely NOT dvar-controlled at all. **This specific piece of the hypothesis (do shadow maps themselves scale with render-scale) remains genuinely open, not confirmed or refuted** -- a real, concrete next RE target if this thread is picked back up, most likely via the same table-driven render-target array (`1404d0740`-`1404d07b0`, all 8-byte-stride name-pointer entries already mapped, shadowmap_large/small sitting between `post_effect_1` and the `ssao` family) rather than more direct-reference searching.
