...
**x86's exact structural counterpart, `FUN_00698f10` (already fully documented, `re_notes/iw5mp.md`-adjacent research this same file), reads the IDENTICAL struct field** (`*(undefined4*)(iVar1+0x1d0)`) as part of its own activator call -- but wraps the whole call in `if (DAT_021d35f4 == '\0') { FUN_0049bf50(...); }`. **Traced `DAT_021d35f4`'s real writer (`FUN_00679260`, via `FindDataWriters.java`) and it is a genuine, real hardware-capability probe**: it iterates 4 candidate depth/stencil format pairs, calling the real `IDirect3D9::CheckDeviceFormat` (vtable+0x30) and `CheckDepthStencilMatch` (vtable+0x28) against `D3DFMT_D24S8` (`0x4b`=75 decimal) combined with the `D3DFMT_NULL` FourCC (`0x4c4c554e` = literal ASCII "NULL", little-endian) -- **this is the well-known, real "NULL render-target" hardware shadow-map capability-detection trick** (a real, documented D3D9-era technique multiple GPU vendors supported, used specifically to query fast hardware shadow-map/depth-buffer-sampling support). `DAT_021d35f4 = 1` when a supported combo is found (i.e., on essentially any real, modern gaming GPU); `= 0` only when NONE of the 4 combos work (old/incapable hardware, or a software/reference rasterizer). **Given `if (DAT_021d35f4 == '\0')` gates the call, this means: on virtually all real player hardware, x86 SKIPS this specific per-light activator call entirely** -- using a faster, hardware-accelerated shadow path that this exact function never even reaches for. The activator-based path was only ever x86's FALLBACK for incapable hardware.
