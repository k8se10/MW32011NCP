...
**Next step**: the live `ResourceUsageLogging=1` diagnostic (already enabled in the deployed `mw3ncp_config.ini` this session, not yet tested) logs real process working-set/private-bytes/pagefile usage once a second -- a live capture during a damage-lag repro at high render scale is the natural next data point, since it can show whether working-set/commit actually approaches a hard ceiling at the moment the stutter hits, which would point toward (1) or (2) above more concretely than further static RE of the CPU-side allocator can.
