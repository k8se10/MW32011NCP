...
- [#2](#2-roadmap-anti-aliasing-and-a-modern-renderer-path-smaa--dlss--path-tracing--dx12vulkan) — ROADMAP: anti-aliasing and a modern-renderer path (SMAA -> DLSS/FSR2 -> path tracing -> DX12/Vulkan) — **Roadmap Idea; SMAA 1x built and shipped 2026-09-21, then PARKED off by default (capture/redraw path itself suspect, independent of the SMAA shaders); FXAA also shipped 2026-09-21 as a limited fallback; `GraphicsApi` config selector + real, SP-only-gated DXVK-loading logic IMPLEMENTED and build-verified 2026-09-23 (DXVK itself not bundled yet); publicly committed to on Nexus 2026-09-23 (Vulkan/RT/DLSS named directly) — see this entry's own "Public commitment" note**
...
**READY FOR A FIRST LIVE TEST, not yet performed** — this is the actual go/no-go milestone for whether DXVK works at all against this specific game, before any Streamline/DLSS work is worth starting: does the game launch and render correctly with `GraphicsApi=Vulkan` set, i.e. does DXVK's own D3D9-to-Vulkan translation work for THIS engine's specific D3D9 usage (this project's own existing hooks — `EndScene`/`Reset`/`CreateDevice` vtable hooks, the full-screen capture/composite pipeline FSR/motion blur/SMAA share — should keep working unchanged regardless of which library produced the underlying vtable, since MinHook operates purely on the in-memory vtable pointer array; genuinely unverified without a live run, not assumed safe). **Real test procedure**: set `GraphicsApi=Vulkan` in `mw3ncp_config.ini` (`[Video]` section), launch `iw5sp.exe`, then check `proxy_d3d9.log` for the `[graphics-api]` line — it will say either "loaded vendored DXVK build from..." (success, watch for the game actually rendering) or one of the real fallback messages (SP-only gate, missing file, or the new missing-export check) explaining exactly why it fell back to `LegacyD3D9` instead. The live game config was deliberately left at the default `LegacyD3D9` rather than flipped to `Vulkan` for this session — this is a genuine "could break the game on launch" architecture change, not a safe visual toggle, so it's left for the user to opt into explicitly when ready to test rather than silently changed underneath an existing session.
