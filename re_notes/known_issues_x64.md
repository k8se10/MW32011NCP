...
Traced the real consumers of the SAVED_SCREEN globals (`DAT_141bb6e00/08/18/1c`) and decompiled the strongest candidate, `FUN_1401dfd80` -- found to be the generic "activate render view N" dispatcher (x64 equivalent of x86's own documented `FUN_0049bf50`), called with a different index per render target/pass, not SAVED_SCREEN-specific in isolation. Added a pure diagnostic hook (`Hook_RenderViewSelectDiag`, zero behavior change) logging on view-index CHANGES only, to see the real activation sequence and whether SAVED_SCREEN's own index appears unusually often specifically during the explosion/downed-state moment. Signature built from raw disassembly (108 bytes, two real wildcarded jump-target operands, the rest confirmed-literal stack-spill stores). x64-only file, build-verified, deployed. **Not yet live-tested** -- the next real data point is a fresh capture with a genuine explosion/downed-state repro at high render scale, checking `[x64-renderview-select-diag]` and the upgraded `[createtexture-storm-diag]` stack-walk output together.
