...
2. **Temporal upscalers (DLSS, FSR 2/3, XeSS).** Not possible from the current D3D9 hook. They need per-pixel motion vectors (the engine has none; only a camera-turn delta exists), a usable depth buffer (`R_RENDERTARGET_FLOAT_Z` exists in the deferred pipeline but is unhooked), and per-frame sub-pixel camera jitter (a projection-matrix change). DLSS additionally targets D3D11/D3D12/Vulkan only and carries NVIDIA NGX redistribution terms.
3. **Path tracing / real RT and DX12/Vulkan.** The realistic route is a D3D9-to-D3D12/Vulkan translation layer with an RTX Remix-style runtime, which supplies the depth, motion vectors and jitter DLSS needs and ships DLSS plus ray reconstruction. It would need engine materials and geometry captured and mapped. One complication to plan for: such a runtime also loads as a `d3d9.dll`, so it would collide with this project's proxy DLL and would need chaining (our proxy forwarding to it instead of the system d3d9.dll). Treat items 2 and 3 as one long-term project, not separate add-ons.
