...
**Follow-up user observation, same session: pausing on these sunny Survival maps shows roughly the SAME ~40fps as live gameplay there, not the normal high pause-menu framerate.** This is not surprising given the already-confirmed pause-menu mechanism above -- it's the expected, consistent behavior, not a new or separate bug: pausing halts simulation but the render pipeline keeps re-drawing the last-known scene, including every active light's shadow work, every frame -- so whatever light-count-driven cost a given map's live gameplay pays keeps being paid at the same rate while paused, rather than dropping to the cheap, mostly-empty pause-menu baseline the campaign mission's own pause showed before its own light count was lower. Real, positive corroboration that the per-light-count-driven cost model holds consistently across both scenarios, not an anomaly.
