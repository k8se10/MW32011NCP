...
- [#4](#4-sp-damagepauselevel-transition-stutter----render-scale-proportional-real-hardcoded-memory-pool-constants-found-genuine-but-do-not-explain-the-full-4gb-ceiling-on-their-own) — SP damage/pause/level-transition stutter, render-scale-proportional — **Investigating; `com_maxfps`-cap theory disproven live for a stuck-30fps Campaign mission; "denser MP scenes" theory CORRECTED/retracted (the original MP test was already a private/low-population match) -- current leading angle is a genuine `iw5mp.exe`-vs-`iw5sp.exe` binary/build difference, not content density; render scale enabled under MP (2026-09-23), warning text still known-inaccurate for MP**
