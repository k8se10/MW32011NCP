...
**Status:** Resolved, confirmed live. Glyph detection itself now confirmed working under a non-English game language by the project owner directly (switched Steam's language locally to retest, after backing up `zone/english` first). One separate, real, lower-priority issue found during that same retest -- see the new round below.
