...
## 70. Corner Friends/Back/Exit hints land in the wrong position at non-16:9 resolutions — 2026-08-08

**Status:** Fixed, rebuilt, **not yet live-tested**. Live-reported by the project owner directly: "major scaling issues at small resolutions with the corner friends/back/exit hints." Repro confirmed at 800x600 (4:3); symptom confirmed as "wrong position entirely," not overlap/clipping.

**Root cause, confirmed by direct code read**: `GetResolutionScale()` (`overlay_hud.cpp`) computes `scaleX`/`scaleY` INDEPENDENTLY -- `realWidth/1920`, `realHeight/1080`. At any 16:9 resolution (every resolution this project has ever been live-confirmed at: 1080p, 1440p, etc.) these two are always identical, so this went unnoticed for the project's entire history. At a non-16:9 aspect ratio they diverge (800x600: `scaleX`=0.417, `scaleY`=0.556) -- this non-uniformly STRETCHES the whole 1920x1080 design-space canvas this project's overlay draws into, which shifts every hardcoded design-space position (like the corner hints' own `kStandardCornerHintX`/`Y`, and the custom Options screen's whole layout) away from where it's actually supposed to land, in a way that scales with how far the real aspect ratio is from 16:9.

**Explicit direction: "scaling must be aspect and res safe for all screens."** Fixed at the root, not per-constant: `ApplyAspectSafeViewportForOverlayDraw()`/`RestoreRealViewportAfterOverlayDraw()` (new, `overlay_hud.cpp`) wrap this project's ENTIRE per-frame overlay draw pass (`Hook_EndScene`) in a temporary, uniformly-scaled, screen-centered "safe area" viewport -- sized to exactly `1920*scale x 1080*scale` where `scale = min(realWidth/1920, realHeight/1080)`, the same "fit without distortion, letter/pillarbox the rest" principle already used elsewhere in this project for the Controller-tab diagram photos (`DrawControllerBodyImage`). Implemented via a REAL `IDirect3DDevice9::SetViewport` call (new `kSetViewportVtableIndex`/`SetViewport_t`, vtable index 47, confirmed against the standard D3D9 COM layout the same way every other index in this file already is), not by threading a manual offset through every draw call site: D3D9 applies the CURRENT viewport's X/Y origin to pretransformed (`D3DFVF_XYZRHW`) vertices automatically, so temporarily pointing the device at this safe-area sub-rect means every existing `DrawGenericTexturedQuad`/`DrawGradientQuad`/etc. call already lands in the correct final screen position with ZERO changes to any of this project's ~50 existing draw call sites. `GetResolutionScale`/`GetRealScreenSize` also needed no changes: called from inside the safe-area viewport, `GetViewport` naturally reports that sub-rect's own Width/Height, which by construction always satisfies `Width/1920==Height/1080==scale` -- an already-uniform `scaleX`/`scaleY` for free.

**At any 16:9 resolution this safe area is EXACTLY the full real viewport** (already `scaleX==scaleY`, zero letterbox needed) -- a fast-path check (`fabsf(safeWidth - realWidth) < 0.5f`) skips the `SetViewport`/restore dance entirely in that case, both to avoid pointless per-frame device-state churn and so a `SetViewport` failure can never regress the one case already live-proven correct across this whole project's history. At any other aspect ratio, the overlay is centered and undistorted instead of stretched into the wrong place.

**Diagnostic logging also added** (`GetRealScreenSize`'s own log line now includes `vp.X`/`vp.Y`, previously never read anywhere in this file) in case this theory turns out incomplete -- if the real engine ITSELF also letterboxes its own UI at non-16:9 resolutions via a non-zero viewport origin (a real, not-yet-confirmed possibility given this engine has a genuine `ui_r_aspectratio` dvar), that would show up directly in `proxy_d3d9.log` as a nonzero `vp.X`/`vp.Y` at the moment this hook's own `EndScene` runs.

Rebuilt `proxy_d3d9.dll` and `ui_hot.dll` -- both compile clean, 0 warnings/0 errors. **Not yet live-tested** -- needs confirmation at 800x600 (the confirmed repro) and ideally an ultrawide/21:9+ resolution (the opposite letterbox direction, pillarboxed left/right instead of letterboxed top/bottom) before this is called closed.

---

## 71. Some players see no controller-glyph icons at all, even on English with default settings — 2026-08-08, investigating

**Status:** Investigating. Root cause not confirmed. One real, confirmed-relevant bug found and fixed (hardcoded XInput slot); a diagnostic escape hatch shipped to help narrow down whether that's the WHOLE story. **Confirmed NOT reproducible on the developer's own machine** (direct quote: "i did test english its fine for me its not reproducible here") -- this is a per-environment issue, not a universal regression.
...
Rebuilt `proxy_d3d9.dll` and `ui_hot.dll` -- both compile clean, 0 warnings/0 errors. **Not yet live-tested.** Needs: (1) confirmation the XInput multi-slot fix alone resolves it for at least one previously-affected reporter, ideally including an x360ce-using setup specifically; (2) if not, `ForceGlyphOverlay=1` test results from a still-affected user to determine whether the resolver-based hypothesis needs its own investigation round.
