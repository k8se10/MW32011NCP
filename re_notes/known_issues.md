...
**Status:** Resolved (detection side). All three confirmed literal-English-text detection bugs (Reload, Quit, Leaderboards) now compare against the game's OWN live-resolved localized text instead of a hardcoded English word -- correct automatically for every language the game supports, not just Italian, and not a per-language guess. The previously-flagged "unconfirmed risk" around key-name text (SPACE/ENTER/F5/Left Mouse/etc.) is now CONFIRMED NOT a bug via decompile -- see below. Not yet live-tested against an actual non-English game client (none available on this machine). Separate, broader initiative for translating this project's OWN added UI text (as opposed to fixing detection) tracked in issue #69.
...
**Key-name text (SPACE/ENTER/F5/"Left Mouse"/"G or Middle Mouse" etc. in `ResolveGlyphAssetNameForKeyName`/`ResolveMenuGlyphAssetNameForKeyName`) -- risk CLOSED, confirmed safe, not guessed.** Decompiled the real keynum-to-display-name chain (`FUN_004bea00` -> `FUN_004bb000`, the same function `KeynumToDisplayName` in `real_settings.h` already wraps) and confirmed it calls NEITHER `FUN_00532230` NOR `FUN_0046df70` -- it returns raw uppercased ASCII for ordinary alphanumeric keys and looks up special keys (Space, Enter, F-keys, mouse buttons, etc.) in a fixed static table (`PTR_s_KEY_TAB_00929c78`) baked directly into the executable, completely outside the general string-localization system. **Key/bind display names are never translated by this engine, confirmed, not assumed** -- these resolvers needed no fix.

Rebuilt `proxy_d3d9.dll` (Release/Win32) -- compiles clean, 0 warnings/0 errors, redeployed to the game root. Not compiled into `ui_hot.dll` (the harness doesn't build `analog_input_hooks.cpp` at all). **Not live-tested against an actual non-English game language** -- there is no non-English asset pack on this machine to test against locally (only `zone/english/` is installed; Steam appears to only download the currently-selected language's fastfiles); needs confirmation from the original Nexus reporter or another non-English player on the next build.
