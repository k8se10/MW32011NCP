...
resolved. Last updated 2026-08-03 (issues #62/#63 round 2/3 follow-ups: auto-mantle's
render/tick timing fix, vibration's sustain/decay envelope, the new Survival armor
diagnostic). *(Prior correction, 2026-08-01 — this line had drifted to
...
## 62. Auto-mantle while sprinting — NOT WORKING, shipped disabled for v0.3.0 (2026-08-03)

**Status:** Open / Not Working. Two rounds attempted the same day (jump-spam regression fixed, then a total-failure regression addressed with a timing fix) but the feature has not been confirmed actually working after either round. **Decision (2026-08-03, v0.3.0 packaging): ship as a documented non-functional feature, `AutoMantleEnabled=0` by default, rather than as a working opt-in.** Do not describe this as "implemented" or "pending playtest" in user-facing docs until a live test actually confirms it fires near a real ledge — treat it as not implemented for now.
...
**Status:** Investigating/in progress, paused here by explicit user request (2026-08-05: "here we hold for now"). Controller/Look/Voice tabs plus the Stick/Button Layout drill-down diagrams are live and working, invoked from all three real Options entry points (pause menu + both main-menu screens). Real, tracked work still open when this resumes:
- **Positional calibration, both diagram pages, not finished**: Stick Layout has all 6 labels drag-calibrated; Button Layout only has 6 of 11 (FIRE/AIM DOWN SIGHT/THROW FRAG/THROW TACTICAL/SPRINT-BREATH still on the computed fallback); Xbox Modern and PS4 have zero drag data on either page (Xbox 360 only).
- **Video/Audio/AdvancedVideo/Movement/Actions tabs still not built** -- the screen only covers Controller/Look/Voice; the "take control of all in-game options" scope explicitly includes these.
- **`mw3ncp_config.ini` sync still not wired to anything live** -- `SyncVanillaSettingsToIni`/`RestoreVanillaSettingsFromIni` (built earlier, `mod_config.cpp`) are compiled and callable but never actually invoked from anywhere; "ini options" being fully under this screen's control needs that connected.
- **The 3 real invocation points have never been live-tested together** -- pause menu was live-tested and working before today's restyle; the two main-menu entry points (`CAMPAIGN_BUTTON_LIST`/`SPECOPS_BUTTON_LIST`) were added today from static `.menu` analysis only, not yet confirmed against the actual running game. "Making sure all in-game options buttons call our screen, not just pause" is the explicit ask for next time this resumes -- verify these three, and watch for any FOURTH real entry point this session's `.menu` trace might have missed.

Older status history (rounds 1-5, the full-scope pivot, the render-suppression dead-end, the harness/HMR build, the 2026-08-05 restyle -- panel/diagram/label-position work) preserved below, dated, oldest first.
...
## 67. `proxy_d3d9.log` has grown to 22GB — found incidentally while diagnosing issue #66, NOT YET FIXED

**Status:** Open, not yet investigated further or fixed -- found as a side effect of pulling this log to diagnose the issue #66 pause-menu-open report above, not something this session set out to look for.

**Finding**: `proxy_d3d9.log` in the game install root is **22,476,092,598 bytes (~22GB)** as of 2026-08-05. Its content is dominated by extremely high-volume, per-frame/per-menu-item diagnostic log lines this project added during earlier live-menu-structure research passes -- `[hud-glyph-pos]`, `[list-item-diag]`, `[localvarint-diag]`, `[localvarstring-diag]`, `[ordinal-hypothesis-diag]`, etc. (see issue #51 and the menu-focus/A-glyph research entries earlier in this file for where these were originally added). These appear to log unconditionally, every single frame, for every menu item/text draw the game does -- on the main menu alone (visible in the tail sampled while diagnosing issue #66) this is dozens of lines per frame, indefinitely, for the lifetime of every play session since whichever pass added them.

**Not yet root-caused to a specific call site or given a fix** -- this entry exists to make sure it doesn't get lost, not because it's been resolved. Likely fix shape (not yet implemented, needs confirming against the actual call sites first): these diagnostic logs were almost certainly meant to be temporary, one-session-only instrumentation for their original investigations (matching this project's own "TEMPORARY debug scaffolding" pattern already used elsewhere, e.g. `EnsureDebugMarkerTexture`'s own comment) and should either be removed now that those investigations concluded, or gated behind a config flag / compile-time switch that defaults off, rather than left unconditionally logging every frame indefinitely. A 22GB (and still growing) log file is a real disk-space risk for players running this mod long-term, independent of anything else in this entry.
...
**Status:** Partially Resolved. One confirmed, structurally-guaranteed bug (the Reload hint) fixed via a language-independent signal already precedented in this same file. Two more suspected-but-unconfirmed bugs (Quit/Leaderboards corner hints) documented, NOT fixed -- fixing them safely needs either a live capture from an affected non-English client or the actual localized asset files, neither of which was available this session.
...
- Line ~6085 (pre-fix): `_stricmp(trimmedText, "Reload") == 0` -- the entire bare-word Reload reminder (no `^N...^7` key-name span exists in this string at all, so there was nothing else to key off) was gated on the literal English word "Reload." Once MW3's own localization renders this as anything else (e.g. Italian would very plausibly show something like "Ricarica"), the comparison always fails and the whole hint -- icon AND text -- silently never fires. This is a real, everyday, unavoidable gameplay hint (every single reload), not an edge case.
- Line ~6201/~6219: `strcmp(param_1, "Quit") == 0` / `_strnicmp(param_1, "Leaderboards", 12) == 0` -- the same pattern for two menu corner-hints. **Not yet fixed** (see below).
- `ResolveGlyphAssetNameForKeyName`/`ResolveMenuGlyphAssetNameForKeyName` (~line 7725-7840) additionally key off literal extracted bind-display text (`"Left Mouse"`, `"ENTER"`/`"RETURN"`, `"G or Middle Mouse"`, `"F5"`, `"SPACE"` via the `isMantleHint` check). **Not confirmed to actually be a bug** -- physical keyboard key labels are commonly left untranslated even in fully localized Western game UIs (this engine's own key-bind display strings may come from a fixed ASCII table outside the general string-localization system, not the same pipeline as full sentences), and this project has no local access to any non-English MW3 asset pack to check directly (see below). Flagged here as an open risk, not fixed blind.

**Why this couldn't be fully verified locally**: this Steam install's `zone/` directory contains only an `english/` language folder (`zone/english/localized_common_mp.ff`, `localized_ui_mp.ff`, etc.) -- no other language pack is present to inspect. Steam appears to only download the fastfiles for the client's currently-selected language, so confirming exactly which UI strings Italian (or any other language) actually uses would require either a live capture from an affected player's own game, or downloading/mounting that language's depot specifically to extract its real localized strings -- neither done this session. Guessing real translated strings and shipping them as match targets was deliberately avoided (this project's own standing "no guessing, verify live" principle) since a wrong guess is no better than the current bug.

**Fix applied (Reload hint only)**: replaced the literal `"Reload"` text match with the SAME structural-signal technique already precedented in this exact file for the Quit/Leaderboards corner hints (see BUG-006, above in this same issue history) -- match on the hint's own real, already-documented screen position instead of its translatable text. This prompt's real `param_3` (626) was already known and documented (in the surrounding code's own comment about its row differing from pickup/buy-station's shared 718) as a genuinely distinct value from every other hint sharing this font/no-color-span branch, so `fabsf(param_3 - 626.0f) < 20.0f` is a real, language-independent discriminator, not a new guess. The actual displayed word ("Reload", "Ricarica", etc.) is still read live from the real string and was never hardcoded in the *displayed* text -- only the *detection* used the English word, and only that part changed.

**Not fixed, needs live data from an affected non-English client before it safely can be**: the Quit/Leaderboards corner-hint matches both already use a positional gate (`looksLikeCornerHintRow`, the row shared by every corner hint) as a precedent for exactly this kind of fix, but that row is shared by BOTH of them (plus Back/Friends, which take a different code path) -- position alone doesn't distinguish Quit from Leaderboards the way it uniquely identified Reload. Telling them apart without their own literal text needs either their real, distinct `param_2` (X position) values (not yet captured) or some other real discriminator (e.g. which real screen/group is focused) -- worth investigating next, but not guessed here. Separately, even once DETECTION is fixed, the corner-hint's own DISPLAYED replacement text is currently hardcoded English (`RequestMenuHintOverlay(..., "Quit", "", bAsset)` / `"Leaderboards "`) -- unlike Reload's word (which is read live), these labels would still show the English word even after a detection fix, which is a separate, smaller cosmetic gap (real localized replacement text for this project's own added labels is out of scope for this pass).

Rebuilt `proxy_d3d9.dll` (Release/Win32) -- compiles clean, 0 warnings/0 errors. Not compiled into `ui_hot.dll` (the harness doesn't build `analog_input_hooks.cpp` at all). **Not live-tested against an actual non-English game language** -- there is no non-English asset pack on this machine to test against locally; needs confirmation from the original Nexus reporter or another non-English player on the next build.

Rebuilt both `proxy_d3d9.dll` (Release/Win32) and the harness's `ui_hot.dll` (Debug/Win32, compiles the same `overlay_hud.cpp` directly) -- both compile with 0 warnings/0 errors, `d3d9.dll` redeployed to the game root. **Not yet live-tested against the real game** -- the crash's root cause is concretely identified and fixed (a verifiably wrong vtable index used with a verifiably wrong argument count, not a guess), and the shader path is a standard, well-established D3D9 pattern, but per this project's own "Verify Live" standard this isn't closed until confirmed live: does the menu open without crashing, and does the blur actually look right (not too subtle/too strong, no visible seams at the panel edge).
