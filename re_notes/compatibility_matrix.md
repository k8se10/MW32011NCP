...
| 11 | Iron Lady | 2 | not yet identified | ⚠️ | AC-130 gunship sequence: flight/camera/fire confirmed working on controller (2026-07-23 live playtest); gun-type switching (105mm/40mm/25mm) does not work — see `known_issues.md` issue #40. Rest of mission not yet separately tested. |
...
**RESOLVED 2026-07-31, shipped in v0.2.5**: Crouch intermittently failing to
fire (~2% of attempts, recovered after pause/unpause or an unrelated button
press — bug #2, `known_issues.md` #27/#42) is fixed and user-confirmed live
(verify+retry the real stance toggle against the real stance field instead of
...
| Mode | Status | Notes |
|---|---|---|
| Survival (all maps) | ⚠️ | Works well overall across maps tested. **Both of this row's previously-listed blockers are now RESOLVED (2026-07-19) — corrected below, not left stale from 2026-07-18:** (1) Predator missile killstreak launch — **FIXED and LIVE-CONFIRMED**, see `known_issues.md` issue #29: the real `notifyonplayercommand` delivery needed an explicit bind-table index argument on the queued client command (`"n 1"`, not bare `"n"`); (2) Sprint's Extreme Conditioning perk override (task #9) — **resolved for free**, see `known_issues.md` issue #6: Sprint was migrated onto its real native kbutton, which makes the engine's own native sprint-duration timer (and Extreme Conditioning's real override on top of it) apply automatically, with no detection code needed at all. **One real blocker remains**: Predator Missile's post-fire missile-guidance phase (controlling the flying missile) still breaks controller input — real reader chain found (`FUN_004554d0`), a live diagnostic is deployed pending one more data pull, see `known_issues.md` issue #30. |
