...
each one is blocked). **Corrected, not deleted, per this file's own
standard: this paragraph was already stale by the time it was written**
(Reload and the five menu corner hints were confirmed working later the
SAME day, per row #34's own detail text) **and Survival ready-up is now
ALSO covered as of 2026-09-14** (a substitute safety signal closed the gap
for that one hint specifically — see row #34's own newest update). Only
buy-station and Sentry-Place remain genuinely absent as of this line's own
latest edit.
...
| 17 | Killstreak: Predator Missile — post-fire guidance aim | Already broken/partial on x86 itself (known lead, not fixed) | **INVESTIGATED (2026-09-14), diagnostic build-verified, NOT YET LIVE-TESTED — still open** | Full native chain independently re-mapped on x64 (`FUN_14011f8e0`→`FUN_140016620`→`FUN_1400168a0`→`FUN_140014dc0`, the last a compiler-fused equivalent of x86's separate `FUN_004554d0`+`FUN_006423d0`, byte-identical `clientStruct+0xc`/`+0x10c`/`+0x110`/`+0x114` offsets, confirmed via fresh `FindConstantRefs.java`+decompile, not assumed from x86). GSC re-verification against the current x64 zone content was attempted (per this project's GSC-first directive) but blocked by a real, newly-discovered OpenAssetTools Unlinker crash affecting any real-content zone project-wide (see `known_issues_x64.md`); fell back to this project's own pre-2026-09-03 decompiled `1555.gsc`, independently re-read and re-confirmed (zero per-frame input reads in the guidance loop, 100% native). Genuinely new finding beyond x86: unlike row #16 above (DPV/Mortar/Turret), missile guidance's own flag is a structurally different field from the flat bit that skips `Hook_MovementTick` for those three — this project's own look injection likely keeps running normally through guidance, meaning the bug may already be partially/fully fixed by the existing pipeline, or may still need a direct write; genuinely unresolved without live data. A new, safer diagnostic hook (`Hook_MissileGuidanceDispatchX64`, learns from x86's own Hold-Breath regression here) is installed, build-verified (x64 `/t:Rebuild` 0 errors, `8664 machine (x64)` fresh timestamp, Win32 regression 0 errors), zero behavior change — not yet live-tested. See `known_issues.md` issue #30's 2026-09-14 round and `known_issues_x64.md`'s own 2026-09-14 "Predator Missile guidance" entry for the full trail |
...
| 37 | Custom mouse cursor overlay | Confirmed working | **FIXED (2026-09-13), not yet live-tested** | Was blocked on the 2026-09-04/05 `#if defined(_M_X64)` early-return stub (`DrawCustomCursorIfNeeded`, `overlay_hud.cpp`), added after the raw x86-only `kCursorVisibleFlagAddr`/`kCursorUiStateAddr` reads were found to silently SEH-swallow an access violation on x64. Resolved same day via `analog_input_hooks_x64.cpp`'s new `kCursorGateSignature`: `DAT_142615b20` (the x64 UI-state global, independently already confirmed elsewhere in this file as `DAT_01c0ad14`'s real equivalent via `SetMenuState`'s own literal mode writes) led to `FUN_14029d170`, the x64 native cursor-draw dispatcher — structurally identical to x86's `FUN_00478540` (same visFlag/uiState gate shape, same `"sp_acceptinvite_warning[_nosave]"` string check, same `"ui_cursor"`-asset draw call) and confirmed via IDENTICAL struct-offset arithmetic (`DAT_14260506c`/position pair sit at the same +0x1c/+0x10 offsets from their UI-context base that x86's own `DAT_01c00474`/`DAT_01c00468`/`046c` do). Exposed via `TryGetCursorGateX64()`, fails closed. A second, previously-invisible bug found while wiring this: the function's own `IsMenuActive_Exported()` gate call is x86-only real (its x64 stub always returns `false`), which would have made the WHOLE function a permanent no-op on x64 outside the glyph-position editor — fixed by branching to the real, already-resolved `IsMenuActiveX64_Exported()` on that platform. Build-verified both platforms (x64 `/t:Rebuild` 0 errors, `dumpbin`-confirmed `8664 machine (x64)` fresh timestamp, Win32 regression rebuild 0 errors, x64 redeployed last); **not yet live-tested**. **UPDATE 2026-09-14 (live playtest): confirmed working at PAUSE, but NOT at the true MAIN MENU** — native cursor showed instead, root-caused via direct log correlation to `IsMenuActiveX64_Exported()` (the `g_menuActiveGateFlag`/`DAT_1406e2550` bit-0x10 gate) reading `0x0` continuously while genuinely at the main menu. Fresh Ghidra decompile of the gate's real writer (`FUN_14007f3b0`, called from `FUN_14029f3f0`, the confirmed x64 `SetMenuState` equivalent) found a real, static-callgraph-confirmed gap: every other menu-opening case (`pausedmenu`/`briefing`/`victoryscreen`/`pregame`/`endofgame`/`coop_lobby`/`levels_challenge`/`main_specops`) has a real caller passing that literal mode value, but case `0xd` (`"main_text"`, the real native main-menu screen — confirmed via `RawStringScan.java` to have exactly one reference in the whole binary, the switch case itself) has ZERO static callers anywhere in `iw5sp.exe` — matching the live evidence exactly. Cross-checked against x86: x86's own equivalent gate (`IsMenuActive()`) IS confirmed via live playtest (`known_issues.md` issue #22) to read true at the real main menu, so this is a genuine x64-specific gap, not an inherited x86 limitation. **Fixed** by OR-ing in `GetMenuStackDepthX64_Exported() > 0` (the real x64 menu-stack-depth primitive, already used elsewhere for the confirmed-working A-glyph/F2-F3 editor, structurally identical to x86's own `GetMenuStackDepth()` which x86's own docs confirm stays nonzero at the main menu) alongside the existing `IsMenuActiveX64_Exported()` check in `DrawCustomCursorIfNeeded` — pause behavior unchanged (still covered by the original gate), main menu now additionally covered. Build-verified both platforms (x64 `/t:Rebuild` 0 errors, `dumpbin`-confirmed `8664 machine (x64)` fresh timestamp, Win32 regression rebuild 0 errors, x64 redeployed last); **not yet live-tested**. Full trail: `re_notes/known_issues_x64.md` issue #1's own "Custom mouse cursor overlay" round |
...
| # | Feature | x86 status | x64 status | Evidence |
|---|---|---|---|---|
| 43 | `InternalRenderScalePercent` | Confirmed working live (real GPU cost scaling) | **LIVE-CONFIRMED 2026-09-14** | Was blocked across two dedicated attempts (indirect/function-pointer call, resisted static xref tooling). Resolved 2026-09-12: `FUN_1401bd1d0` confirmed as the real x64 equivalent of x86's `FUN_00679010`, hooked via MinHook, same override-before-native-call design as x86. This paragraph is intentionally left in place per this doc's own convention — only this annotation records the fix |
