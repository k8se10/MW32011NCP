...
| 46 | `ForceAnisotropicFiltering` | Confirmed working live | **CORRECTED (2026-09-23) — actually ABSENT, a real overclaim** | `ApplyForcedAnisotropicFilteringIfEnabled` (`overlay_hud.cpp`) itself has no arch guard and always logs `[aniso-force]`, but it calls `SetDvarFloat("r_texFilterAnisoMax"/"r_texFilterAnisoMin", 16.0f)` (`real_settings.cpp`), and `SetDvarFloat`/`SetDvarBool`/`SetDvarString` (lines 117-134) are ALL unconditional `#if defined(_M_X64) \|\| defined(_WIN64)` no-ops on x64 (added 2026-09-04 to stop a crash from the raw x86-only function pointers `SetDvarFloatRaw`/etc., hardcoded addresses like `0x005513c0`) — the log line fires regardless, giving a false impression the write happened. No x64 equivalent of these setters has ever been built (only read-only `GetDvarFloatX64`/`GetDvarStringX64` exist, `analog_input_hooks_x64.cpp`). This row's original "PRESENT"/log-evidence verdict was carried over from x86-era testing, never actually re-verified on x64 — same documentation-drift bug class this file's own 2026-09-13 correction round already flagged once. Not fixed — needs real x64 RE of `SetDvarBool`/`SetDvarFloat`'s native equivalents before this (or the two rows below) can work. Tracked: `known_issues_x64.md` issue #6 |
| 47 | `ForceHighQualityShadows` | Confirmed working live | **CORRECTED (2026-09-23) — actually ABSENT**, same root cause as row 46 (`SetDvarBool("sm_fastSunShadow", 0)` silently no-ops on x64) | See row 46 |
| 48 | `ForceHighQualityLighting` | Confirmed working live | **CORRECTED (2026-09-23) — actually ABSENT**, same root cause as row 46 (`SetDvarBool("r_cacheModelLighting"/"r_cacheSModelLighting", 0)` silently no-ops on x64) | See row 46 |
