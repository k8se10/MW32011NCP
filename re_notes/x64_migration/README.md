...
1b. **Movement/look pipeline re-located, high-confidence static match, NOT yet
    ...
    | `FUN_14007de20` | Alternate-mode angle path, called conditionally | **CORRECTED 2026-09-14 (was "not yet matched" here originally): this IS the real x64 match for issue #30's long-tracked "third analog input channel"** (x86 equivalent: `FUN_0057e360`, DPV/Goalpost-mortar/Goalpost-M2-turret aim) — confirmed via full decompile (`re_notes/ghidra_scripts/decomp_dpv_channel_candidates_x64.txt`): gated on the same per-client bit (`DAT_1406e4774`+player*0xce5c, bit 0x80000) x86's `+0x1094` bit 0x80000 used, and floor-packs a scaled raw-mouse-delta read into cmd+0x3e/0x3f. Fixed this session (`analog_input_hooks_x64.cpp`'s `Hook_MountedAimTick`) — see `re_notes/known_issues_x64.md`'s 2026-09-14 entry and `re_notes/known_issues.md` issue #30 for the full trail. NOT the vehicle-camera path guessed originally. |
    | `FUN_14007e4e0` | Separate double-buffered analog channel, called conditionally alongside `FUN_14007de20` | **CORRECTED 2026-09-14 (this row previously guessed this WAS issue #30's channel — it is not, see the row above)**: this is actually the x64 equivalent of x86's `FUN_0057df60` (cursor-placement/vehicle-driving-bit mode dispatch, branch 3 of the orchestrator) — a different bug/feature entirely (precision_airstrike-style cursor placement, or the vehicle-driving usercmd bit). Not the target of the 2026-09-14 fix. |
