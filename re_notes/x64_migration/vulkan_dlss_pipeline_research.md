...
17. **NEW, 2026-09-24 — Stage 1 view-matrix RE started: a real, strong candidate for camera position + forward/right/up basis vectors found via direct decompile, not yet live-confirmed.** Direct instruction to start the view-matrix RE work — the one piece `sl::Constants` needs (§2.5) that this project's own existing per-tick look-angle data (motion blur's yaw/pitch delta) doesn't cover: real world-space camera position and full basis vectors, not just a rotation delta.
    ...
    - **ROUND 1 LIVE RESULT, same day — partial confirmation, partial miss.** `+0x1590/1594/1598` (candidate camera **POSITION**) is **real, live-confirmed**: `[0,0,0]` at the main menu (correct default), then real, changing world-space coordinates once in a level — `[-398.6 2826.2 -89.6]` → `[-437.2 2776.6 -86.1]` → `[19.5 2450.2 -194.97]`, consistent with genuine player/camera movement. The three candidate basis-vector offsets (`+0x1580/1550/1560`, inferred purely from the frustum-corner USAGE pattern in `FUN_1401e0880`, never independently confirmed against real data) all read exactly `[0,0,0]` on every single fire, including fires where position was already real and nonzero — **that specific offset guess is wrong**. Forward/right/up live somewhere else in the same wholesale-copied `+0x1490..+0x1630` block, or are computed/populated by a different function this session hasn't traced yet.
    - **ROUND 2, same day, in progress**: rather than re-guess more single offsets blind, a one-shot full-block dump (`[x64-view-block-dump]`, same hook) now logs all 104 floats of the `+0x1490..+0x1630` span raw, the first time position is confirmed nonzero (i.e. genuinely in-level). Real basis vectors should show up as float3 triples with magnitude roughly 1.0 somewhere in this real capture — eyeball-searchable from live data instead of guessing further offsets. Build-verified and deployed; awaiting the next live capture.
    - **Next step once basis vectors are found**: convert `(fwd, right, up, pos)` plus the already-known projection-matrix layout into a real camera-to-world matrix, feed it through `sl_matrix_helpers.h`'s `calcCameraToPrevCamera`/`recalculateCameraMatrices` (§2.5) for the previous-frame comparison `clipToPrevClip`/`prevClipToClip` Streamline needs, retaining one frame of history.
