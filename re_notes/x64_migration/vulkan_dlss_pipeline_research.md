...
17. **NEW, 2026-09-24 — Stage 1 view-matrix RE started: a real, strong candidate for camera position + forward/right/up basis vectors found via direct decompile, not yet live-confirmed.** Direct instruction to start the view-matrix RE work — the one piece `sl::Constants` needs (§2.5) that this project's own existing per-tick look-angle data (motion blur's yaw/pitch delta) doesn't cover: real world-space camera position and full basis vectors, not just a rotation delta.
    ...
    - **Not yet live-confirmed** — this RE reading, while internally consistent (three plausible orthogonal-ish basis vectors plus a homogeneous position point, computed exactly where frustum-corner math would need them, on the same struct the confirmed projection matrix lives in, populated immediately before it every frame), is still a decompile-only interpretation, not a live-verified one. A real, read-only diagnostic hook (`[x64-view-matrix-diag]`, `Hook_ProjectionMatrixBuild`, `analog_input_hooks_x64.cpp`) now logs these four float3s on the same cadence as the existing `[x64-proj-matrix-diag]` line (first 5 fires, then every 5000th) — zero behavior change, this IS the live-verification step. Expected live signature if the reading is correct: `fwd`/`right`/`up` should look roughly unit-length and rotate together as the player looks around; `pos` should track real world-space movement (not a small, bounded, angle-like range).
    - **Next step once live-confirmed**: convert `(fwd, right, up, pos)` plus the already-known projection-matrix layout into a real camera-to-world matrix, feed it through `sl_matrix_helpers.h`'s `calcCameraToPrevCamera`/`recalculateCameraMatrices` (§2.5) for the previous-frame comparison `clipToPrevClip`/`prevClipToClip` Streamline needs, retaining one frame of history. Not yet started.
