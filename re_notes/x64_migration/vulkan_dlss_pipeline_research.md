...
2. **Sub-pixel jitter injection — REVISED 2026-09-23, contract fully specified, native RE still not started.** The exact data contract (separate `jitterOffset` field, un-jittered matrices, the Halton(2,3)/render-resolution formula, the mip-bias companion requirement, a real methodology precedent for validating it via a from-scratch TAA test rig) is now fully researched (section 2.3) — the one real remaining item is finding and hooking IW5's own projection-matrix-build function (one plausible, unconfirmed lead on record: `FUN_1401d8f70`) or, as a lower-native-RE-burden alternative, intercepting the relevant `SetVertexShaderConstantF` register directly. Not resolved, but no longer an open research question — a scoped RE task now. **Real context added, 2026-09-24** (`renderer_map.md`, the same session's full renderer-mapping pass): `FUN_1401d8f70` is now confirmed to sit inside `FUN_1401d7480`'s per-player frame-setup chain — main-thread, pre-command-buffer, alongside the rest of the real camera/view/FOV setup. The function itself is STILL not decompiled — this only confirms its real architectural position (useful for scoping a hook correctly once the decompile happens), not its contents. **This is now the single highest-leverage next RE task for DLSS specifically**: it's the one remaining blocker between "contract fully specified" (true since section 2.3) and "implementable."
...
8. **NEW, 2026-09-24 — FSR and RT status re-checked against the same session's full renderer-mapping/analysis work; neither changes.** Direct instruction to continue the renderer-replacement roadmap ("lets continue with our full renderer replacement by adding our next steps into the visual enhancement suite, RT, DLSS, FSR etc") prompted a deliberate re-check of all four fronts, not just DLSS.
   - **FSR: no new next step, confirmed unaffected.** FSR 1.0 RCAS is a pure post-process spatial upscale running entirely inside this project's own existing full-screen capture/composite pipeline (the same `StretchRect`-based capture motion blur/SMAA already use), AFTER the native scene is fully rendered, using this project's own code exclusively. It has zero dependency on the native command-buffer/backend architecture, the render-target creation chain, or the `EndScene` threading model this session mapped — none of that work changes anything for FSR. Already shipped and x64-ported (issues #94/#103/#104's own required safety gates already closed this out).
